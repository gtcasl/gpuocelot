...
void capModel3Cuda( CudaInvariants* invariants, CudaResult* results )
{
	...
	// configuration
	cudaCheck( cudaGetDevice( &invariants->device ) );
...
/*
	// symbols
	cudaCheck( cudaMemcpyToSymbol( wtab, hostWtab, 128 * sizeof(FLOAT), 
		0, cudaMemcpyHostToDevice ) );
	cudaCheck( cudaMemcpyToSymbol( ytab, hostYtab, 128 * sizeof(FLOAT), 
		0, cudaMemcpyHostToDevice ) );
	cudaCheck( cudaMemcpyToSymbol( ktab, hostKtab, 
		128 * sizeof(unsigned long), 
		0, cudaMemcpyHostToDevice ) );	
*/
	cudaDeviceProp properties;
	cudaCheck( cudaGetDeviceProperties( &properties, invariants->device ) );
	...
	cudaCheck( cudaMallocHost( (void**) &hostInvariants, invariantBytes ) );
	cudaCheck( cudaMallocHost( (void**) &hostResults, resultBytes ) );
	
	cudaCheck( cudaMalloc( (void**) &deviceResults, resultBytes ) );
	cudaCheck( cudaMalloc( (void**) &deviceScratch, scratchBytes ) );
	cudaCheck( cudaMalloc( (void**) &deviceInvariants, invariantBytes ) );
	...
	// copy
	cudaCheck( cudaMemcpyAsync( deviceInvariants, hostInvariants, 
	...
	// copy
	cudaCheck( cudaMemcpyAsync( hostResults, deviceResults, 
	...
	cudaCheck( cudaFreeHost( hostResults ) );
	cudaCheck( cudaFreeHost( hostInvariants ) );
	
	cudaCheck( cudaFree( deviceInvariants ) );
	cudaCheck( cudaFree( deviceScratch ) );
	cudaCheck( cudaFree( deviceResults ) );
