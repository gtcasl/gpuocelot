...
const DirectionalGraph::node_set DirectionalGraph::getOutNodesSet( const node_type& nodeId ) const{
...
const DirectionalGraph::node_set DirectionalGraph::getInNodesSet( const node_type& nodeId ) const{
 ...
 */
int DirectionalGraph::insertEdge( const node_type &fromNode, const node_type &toNode, const bool createNewNodes ){
	/* 1) Tests if can create new nodes */
	if( createNewNodes ){
		/* 1.1) Create ongoing and destiny nodes */
 ...
 */
int DirectionalGraph::eraseEdge( const node_type &fromNode, const node_type &toNode, const bool removeIsolatedNodes ){
	/* 1) Test if both nodes of the edge exist */
	if( nodes.find(fromNode) == nodes.end() ){
		return 1;
	}
	...
	if( removeIsolatedNodes ){
		if( (outArrows[fromNode].size() == 0) && (inArrows[fromNode].size() == 0) ){
			eraseNode(fromNode);
		}

		if( (outArrows[toNode].size() == 0) && (inArrows[toNode].size() == 0) ){
			eraseNode(toNode);
		}
	}
...
std::ostream& DirectionalGraph::print( std::ostream& out ) const{
	out << "digraph DataFlowGraph{" << endl;
