...
executive::GlobalVariable & executive::Executive::getGlobalVariable(const char *symbol) {
	...
	assert(device.globals.count(symbol) != 0);
	return device.globals[std::string(symbol)];
}
...
bool executive::Executive::deviceMemcpyToSymbol(const char *symbol, const void *src, size_t count, 
	...
	GlobalMap::iterator glb_it = device.globals.find(std::string(symbol));
	if (glb_it != device.globals.end()) {
		GlobalVariable & globalVar = glb_it->second;
		if (count + offset <= globalVar.size) {
			char *dstPtr = (char *)globalVar.pointer + offset;
			report("deviceMemcpyToSymbol('" << symbol 
				<< "') - dstPtr: " << (void *)dstPtr 
				<< " (" << count << " bytes)");
			if (getSelectedISA() == ir::Instruction::GPU) {
				if (!cuda::CudaDriver::cuMemcpyHtoD(hydrazine::bit_cast<CUdeviceptr, 
					char*>(dstPtr), src, count)) {
					Ocelot_Exception("Memory copy to symbol failed." );
				}
			}
			else {
...
bool executive::Executive::deviceMemcpyFromSymbol(const char *symbol, void *dst, 
	...
	if (glb_it != device.globals.end()) {
		...
		if (count + offset <= globalVar.size) {
			char *srcPtr = (char *)globalVar.pointer + offset;
...
*/
bool executive::Executive::loadModule(std::string path, std::istream & ptx) {
...
*/
bool executive::Executive::loadModule(std::string filename) {
	std::ifstream file(filename.c_str());
	if (!file.is_open()) return false;
	return loadModule(filename, file);
}
...
void executive::Executive::registerGlobalVariable(const std::string & module, 
	...
	if (m_it == modules.end()) {
		Ocelot_Exception("invalid module - " << module);
	}
	
	ir::Module::GlobalMap::iterator g_it = m_it->second->globals.find(varName);
	if (g_it == m_it->second->globals.end()) {
		Ocelot_Exception("global variable '" << varName 
				<< "' - not declared in module " << module);
	}
	else {
		if (g_it->second.registered) {
			Ocelot_Exception("previously registered global variable '" << varName << "'");
		}
		g_it->second.reference = hostPtr;
	}
...
void executive::Executive::registerTexture(const char *module, const char *name, int dimensions,
	...
	ir::Module::TextureMap::iterator t_it = m_it->second->textures.find(name);
	if (t_it == m_it->second->textures.end()) {
		Ocelot_Exception("texture variable '" << name 
				<< "' - not declared in module " << module);
	}
	
	t_it->second.normalizedFloat = (bool)normalized;
}
...
executive::StringVector executive::Executive::enumerateKernels(const std::string& moduleName) const {
	...
	StringVector names;
	for (ir::Module::KernelMap::const_iterator kernel = module->second->kernels.begin(); 
...
/*!
	Gets a kernel by module and kernel name for the currently selected device.
	This function will perform translation if necessary.
...
*/
ir::Kernel * executive::Executive::getKernel(const std::string& moduleName, const std::string& kernelName) {
	
	ir::Kernel * kernel	= 0;
	Device& device = devices[getSelected()];
	
	ModuleMap::iterator mod_it = modules.find(moduleName);
	if (mod_it != modules.end()) {
		KernelMap::iterator nativeKernel = device.kernels.find(kernelName);
		if (nativeKernel != device.kernels.end()) {
			translateModuleToISA(moduleName, getSelectedISA());
			nativeKernel = device.kernels.find(kernelName);
			if (nativeKernel != device.kernels.end()) {
				kernel = nativeKernel->second;
			}
		}
	}
...
void executive::Executive::launch(const std::string & moduleName, const std::string & kernelName,
	...
	ir::Kernel *kernel = getKernel(moduleName, kernelName);
...
bool  executive::Executive::bindTexture(size_t *offset, const std::string & textureName, 
	...
	ir::Texture* tex_it = getTexture(textureName);
	if (tex_it != 0) {
		ir::Texture &texture = *tex_it;
...
ir::Kernel *executive::Executive::translateToISA(ir::Instruction::Architecture isa, 
	...
	ir::Kernel *kernel = mod_it->second->getKernel(kernelName);
	...
	switch (isa) {
		...
		case ir::Instruction::LLVM:
		{
			...
			llvmKernel->setDevice(&device, hostWorkerThreads);
			//
			// ...
			//
	...
	// insert kernel
	if (translated) {
		device.kernels[translated->name] = translated;
	}
...
*/
void executive::Executive::translateModuleToISA(std::string moduleName, ir::Instruction::Architecture isa) {
	report("Executive::translateModuleToISA(" << moduleName << ")");
	
	ModuleMap::iterator mod_it = modules.find(moduleName);
	assert (mod_it != modules.end()); 
	
	if (getSelectedISA() == ir::Instruction::GPU) {
		if (device.modules.count(moduleName) == 0) {
			translateModuleToGPU(* mod_it->second);
		}
	}
	else {
		ir::Module::KernelMap & target = mod_it->second->kernels;
		for (ir::Module::KernelMap::iterator k_it = target.begin(); k_it != target.end(); ++k_it) {
			if (device.kernels.count(k+it->first) == 0) {
...
bool executive::Executive::translateModuleToGPU(ir::Module &module) {
	...
	assert(device.modules.count(module.modulePath) == 0);
	
	DriverModuleMap::iterator driverModule = device.modules.insert(std::make_pair(module.modulePath, 0)).first;
	
	CUresult cuResult = cuda::CudaDriver::cuModuleLoadData(&driverModule->second, str.c_str());
	if (cuResult != CUDA_SUCCESS) {
	...
	// kernels
	ir::Module::KernelMap &ptxKernels = module.kernels;
	ir::Module::KernelMap &gpuKernels = device.kernels;
	for (ir::Module::KernelMap::iterator k_it = ptxKernels.begin(); k_it != ptxKernels.end(); ++k_it) {
		CUfunction cuFunction;
		if (cuda::CudaDriver::cuModuleGetFunction(&cuFunction, driverModule->second, k_it->second->name.c_str()) != CUDA_SUCCESS) {
	...
	// globals
	for (GlobalMap::iterator g_it = module.globals.begin(); g_it != module.globals.end(); ++g_it) {
		CUdeviceptr ptr;
		unsigned int bytes = 0;
		if (cuda::CudaDriver::cuModuleGetGlobal(&ptr, &bytes, driverModule->second, g_it->first.c_str()) != CUDA_SUCCESS) {
		...
		global.name = g_it->first;
		global.size = bytes;
		global.host_pointer = g_it->second.pointer;
		global.pointer = (void*)ptr;
		device.globals.insert(std::make_pair(global.name, global));
		
		registerGPUGlobal(g_it->second, ptr);
	}
	
	// textures
	for (TextureMap::iterator t_it = module.textures.begin(); t_it != module.textures.end(); ++t_it) {
		CUtexref reference;
		if (cuda::CudaDriver::cuModuleGetTexRef(&reference, module.cuModule, t_it->first.c_str()) != CUDA_SUCCESS) {
			report("failed to get texture from loaded GPU module");
			return false;
		}
		
		device.textures.insert(std::make_pair(t_it->first, reference));
	}
