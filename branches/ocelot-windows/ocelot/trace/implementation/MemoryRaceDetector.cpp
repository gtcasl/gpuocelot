...
namespace trace
{
	...
	void MemoryRaceDetector::_write( const TraceEvent& event )
	{
		...
		for( int thread = 0; thread < threads; ++thread )
		{
			...
			for( ir::PTXU64 byteAddress = *address; 
				byteAddress < *address + event.memory_size; ++byteAddress )
			{
				unsigned int byte = (unsigned int) byteAddress;
				if( _writers[ byte ] != -1 && _writers[ byte ] != thread )
				{
					...
					stream << "\n";
					stream << "Near " << _kernel->location( (unsigned int)event.PC ) << "\n";
	...
	void MemoryRaceDetector::_read( const TraceEvent& event )
	{
		...
		for( int thread = 0; thread < threads; ++thread )
		{
			if( !event.active[ thread ] ) continue;
			for( ir::PTXU64 byteAddress = *address; 
				byteAddress < *address + event.memory_size; 
				++byteAddress )
			{
				unsigned int byte = (unsigned int) byteAddress;
				if( _writers[ byte ] != -1 && _writers[ byte ] != thread )
