...
namespace analysis{
/* BranchInfo holds/points informations of branches required to do the divergence analysis */
class BranchInfo{
	public:
		...
		/*!\brief Tells if a node is control dependent on a divergent branch instruction. */
		...
		const graph_utils::DirectionalGraph::node_type &predicate() const {return *_dfgInstruction.s.begin()->pointer;};
