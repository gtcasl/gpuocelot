...
static int test_malloc3d(bool verbose) {
	...
	cudaExtent extent2;	//for making Memset
	extent2.width = 1 * sizeof(int);
	extent2.height = 4; 
	extent2.depth = 3;

	if (cudaMemset3D(p.dstPtr, 0x0F, extent2) != cudaSuccess) {
	...
	for (int i = 0; i < depth; i++) {
		for (int j = 0; j < height; j++) {
			for (int k = 0; k < width; k++) {
				//printf("%x, ", host[k+j*newWidth+i*height*newWidth]);
