...
define void @_subkernel_convergence_1_opt3_ws4(%LLVMContext* %__ctaContext) nounwind align 1 {
exit:
  ...
  %r0 = load i64* %rt2, align 8
  %r3.vec = sitofp <4 x i32> %insert.threadId.x.t3.vec to <4 x float>
  %r4.vec = fadd <4 x float> %r3.vec, %r3.vec
  %r5.vec = fadd <4 x float> %r4.vec, <float 1.000000e+00, float 1.000000e+00, float 1.000000e+00, float 1.000000e+00>
  %rt5 = sext i32 %threadId.x.t0 to i64
  %r6 = shl nsw i64 %rt5, 2
  %r7 = add i64 %r0, %r6
  %vec.rt6 = inttoptr i64 %r7 to <4 x float>*
  store <4 x float> %r5.vec, <4 x float>* %vec.rt6, align 16
