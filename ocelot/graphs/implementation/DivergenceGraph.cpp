...
/*!\brief Inserts a edge[arrow] between two nodes of the graph / Can create nodes if they don't exist */
int DivergenceGraph::insertEdge( const ir::PTXOperand* origin, const node_type &toNode, const bool createNewNodes ){
	if( createNewNodes ){
...
void DivergenceGraph::computeDivergence(){
	...
	{
		map<const ir::PTXOperand*, node_set>::iterator divergence = _specials.begin();
		map<const ir::PTXOperand*, node_set>::iterator divergenceEnd = _specials.end();
...
string DivergenceGraph::getSpecialName( const ir::PTXOperand* in ) const{
	assert (in->special < ir::PTXOperand::SpecialRegister_invalid);
	return (ir::PTXOperand::toString(in->special).erase(0, 1) + ir::PTXOperand::toString(in->vIndex));
}

/*!\brief Prints the divergence graph in dot language */
std::ostream& DivergenceGraph::print( std::ostream& out ) const{
	using ir::PTXOperand;
	out << "digraph DivergentVariablesGraph{" << endl;

	/* Print divergence sources */
	map<const PTXOperand*, node_set>::const_iterator divergence = _specials.begin();
	map<const PTXOperand*, node_set>::const_iterator divergenceEnd = _specials.end();

	out << "//Divergence sources:" << endl;
	for( ; divergence != divergenceEnd; divergence++ ){
		if( divergence->second.size() ){
			out << getSpecialName(divergence->first) << "[style=filled, fillcolor = \"" << (isDivSource(divergence->first)?"tomato":"lightblue") << "\"]" << endl;
		}
	}
	...
	for( ; divergence != divergenceEnd; divergence++ ){
		if( divergence->second.size() ){
			...
			for( ; node != endNode; node++ ){
				out << getSpecialName(divergence->first) << "->" << *node << "[color = \"" << (isDivSource(divergence->first)?"red":"blue") << "\"]" << endl;
			}
		}
	}
