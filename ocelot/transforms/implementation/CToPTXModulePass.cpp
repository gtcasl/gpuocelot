...
namespace transforms
{
    ...
    void CToPTXModulePass::runOnModule(ir::Module& m)
    {
        ...
        else if(functionName == ACTIVE_THREAD_SUM)
        {
            kernel = activeThreadSum();
        }
        else 
        {
    ...
    ir::PTXKernel * CToPTXModulePass::activeThreadSum()
    {
        ...
        ir::PTXStatement sum = ir::PTXStatement(ir::PTXStatement::Param);
        sum.name = "sum";
        sum.type = type;
        ...
        kernel->arguments.push_back(ir::Parameter(sum, true, true));
