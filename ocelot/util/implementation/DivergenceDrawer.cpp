...
#include <fstream>
#include <assert.h>
#include <ocelot/util/interface/DivergenceDrawer.h>
#include <ocelot/analysis/interface/DirectionalGraph.h>

namespace util {
static const std::string FULLGRAPH_ALIVE_OUT = "aliveOut";
...
/*!\brief Draw edges from the block predecessor to it, and in the (D+C)FG
	edges inside the block from aliveIn to phis or instructions, and
	isntructions to aliveOut */
inline std::string DivergenceDrawer::_edges( 
	const analysis::DataflowGraph::Block &block, const bool isFullGraph ) const{
	...
	DataflowGraph::BlockPointerSet::const_iterator
		endPredecessor = predecessors.end();
	std::string thisBlockName, predecessorName;
	...
	if( isFullGraph && ((thisBlockName != "entry")
		...
		if( block.phis().empty() ){
			...
			buff << thisBlockName << FULLGRAPH_PHI << ":out->"
				<< thisBlockName << FULLGRAPH_INSTRUCTION << ":in;";
		}
		buff << thisBlockName << FULLGRAPH_INSTRUCTION << "->"
			<< thisBlockName << FULLGRAPH_ALIVE_OUT << ';';
	}
...
void DivergenceDrawer::drawVariablesGraph() const{
	_sanityTest();
	std::string fileName = _path + "prof." + _kernelName + "_dataFlow.dot";
	std::ofstream file(fileName.c_str());

	if( file.is_open() != true ){
		std::cerr << "Coldn't open file " + fileName + " for writing.";
		return;
	}
...
void DivergenceDrawer::drawDivergenceGraph() const{
	_sanityTest();
	const std::string fileName = _path + "prof."
		+ _kernelName + "_divergenceFlow.dot";
	std::ofstream file(fileName.c_str());
...
void DivergenceDrawer::drawControlFlowGraph() const{
	_sanityTest();
	std::map<DataflowGraph::RegisterId, std::string> registersLocations;
	...
	if( !file.is_open() ){
		std::cerr << "Couldn't open file '" + _path + "prof."
			+ _kernelName + "_controlFlow.dot' for writing";
		return;
	}
	...
	for( ; block != endBlock; block++ ){
		...
		std::string blockLabel = block->label();

		if( blockLabel == "entry" ){
			file << blockLabel << "[shape=Mdiamond,style=\"filled\","
				"color=\"white\",fillcolor=\"black\",fontcolor=\"white\","
				"label=\"Entry\"];";
			continue;
		}
		...
		DataflowGraph::InstructionVector::const_iterator
			endInstruction = instructions.end();

		if( instruction == endInstruction ){
			std::cerr << "*****************Empty Block: " << block->label()
				<< " N:" << block->id() << std::endl;
			continue;
		}
		...
		file << blockLabel
		    << "[shape=none, margin=0, label=<<TABLE BORDER=\"0\" "
		    	...
		    	"WIDTH=\"600\"><TR><TD COLSPAN=\"2\" BGCOLOR=\"" << bgcolor
		    << "\">" << block->label() << "</TD></TR>";
	...
	file << edgesDot.str() << "legend [shape=none, margin=0, "
		"label=<<TABLE BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\" "
		"CELLPADDING=\"1\">"
	    << "<TR><TD COLSPAN=\"3\"><B>LEGEND</B></TD></TR>"
	    << "<TR><TD><FONT COLOR=\"blue\">blue edges</FONT></TD><TD "
	    	"COLSPAN=\"2\">fallthrough path</TD></TR>"
	    << "<TR><TD>black edges</TD><TD COLSPAN=\"2\">branch path</TD></TR>"
	    << "<TR><TD BGCOLOR=\"tomato\">light red instruction</TD><TD "
	    	"COLSPAN=\"2\">divergent instruction</TD></TR>"
	    << "<TR><TD BGCOLOR=\"tomato\">block label</TD><TD "
	    "COLSPAN=\"2\">Divergent block prediction"
	    << "</TD></TR></TABLE>>];};";
	file.close();
}

/*!\brief Draw the (D+C)FG, draw comparison statistics to profiling results */
void DivergenceDrawer::drawFullGraph() const{
	_sanityTest();
	std::map<DataflowGraph::RegisterId, std::string> registersLocations;
	DataflowGraph::const_iterator block = divAnalysis->getDFG()->begin();
	DataflowGraph::const_iterator endBlock = divAnalysis->getDFG()->end();
	const analysis::DivergenceGraph
		&divergenceGraph = divAnalysis->getDivergenceGraph();

	std::ofstream file(std::string(_path + "prof." + _kernelName
		+ "_fullDFG.dot").c_str());
	if( !file.is_open() ){
		std::cerr << "Couldn't open file '" + _path + "prof."
			+ _kernelName + "_fullDFG.dot' for writing";
		return;
	}
	...
	for( ; block != endBlock; block++ ){
		...
		file << "subgraph cluster" << blockLabel << "{" << blockLabel
		    << "[shape=none,margin=0,label=<<TABLE BORDER=\"0\" "
		    "CELLBORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"1\"><TR><TD"
		...
		if( block->aliveIn().size() > 0 ){
			...
			std::set<DataflowGraph::RegisterId> orderedAliveIn;
			{
				DataflowGraph::Block::RegisterSet::const_iterator
					aliveIn = block->aliveIn().begin();
				DataflowGraph::Block::RegisterSet::const_iterator
					endAliveIn = block->aliveIn().end();
			...
			if( block->phis().size() > 0 ){
				...
				for( ; phi != endPhi; phi++ ){
					...
					for( ; phiS != endPhiS; phiS++ ){
						assert(orderedAliveIn.find(phiS->id)
							!= orderedAliveIn.end());
			...
			std::set<DataflowGraph::RegisterId>::const_iterator
				aliveIn = orderedAliveIn.begin();
			std::set<DataflowGraph::RegisterId>::const_iterator
				endAliveIn = orderedAliveIn.end();
			for( ; aliveIn != endAliveIn; aliveIn++ ){
		...
		file << "</TABLE>>];" << blockLabel << FULLGRAPH_ALIVE_OUT
			<< "[shape=none,margin=0,label=<<TABLE BORDER=\"0\" "
			"CELLBORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"1\">";
		if( block->aliveOut().size() > 0 ){

			std::set<DataflowGraph::RegisterId> orderedAliveOut;
			{
				DataflowGraph::Block::RegisterSet::const_iterator
					aliveOut = block->aliveOut().begin();
				DataflowGraph::Block::RegisterSet::const_iterator
					endAliveOut = block->aliveOut().end();
			...
			std::set<DataflowGraph::RegisterId>::const_iterator
				aliveOut = orderedAliveOut.begin();
			std::set<DataflowGraph::RegisterId>::const_iterator
				endAliveOut = orderedAliveOut.end();
			...
			for( ; aliveOut != endAliveOut; aliveOut++ ){
				...
				const DataflowGraph::Block::RegisterSet&
					aliveIn = block->aliveIn();
				const DataflowGraph::Block::RegisterSet::const_iterator
					ai = aliveIn.find(*aliveOut);
		...
		file << blockLabel << FULLGRAPH_INSTRUCTION
		    << "[shape=none,margin=0,label=<<TABLE BORDER=\"0\" "
		    	"CELLBORDER=\"1\"CELLSPACING=\"0\" CELLPADDING=\"1\"><TR><TD "
		    	"WIDTH=\"60px\">Out<br/>Regs</TD><TD WIDTH=\"580px\" "
		    	"PORT=\"in\">Instruction</TD><TD WIDTH=\"60px\">In<br/>Regs"
		    	"</TD></TR>";
		...
		for( ; instruction != endInstruction; instruction++ ){
			...
			DataflowGraph::RegisterPointerVector::const_iterator
				sReg = instruction->s.begin();
			DataflowGraph::RegisterPointerVector::const_iterator
				endSReg = instruction->s.end();

			DataflowGraph::RegisterPointerVector::const_iterator
				dReg = instruction->d.begin();
			DataflowGraph::RegisterPointerVector::const_iterator
				endDReg = instruction->d.end();
			...
			if( sReg != endSReg ){
				if( (instruction->s.size() != totalLines)
					&& (totalLines > 1) ){
					sRowSpan << " ROWSPAN=\""
						<< totalLines / instruction->s.size() << '"';
				}
		...
		if( block->phis().size() > 0 ){
			...
			file << blockLabel << FULLGRAPH_PHI
				<< "[shape=none,margin=0,label=<<TABLE BORDER=\"0\" "
				"CELLBORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"1\"><TR>";
			...
			for( ; phi != endPhi; phi++ ){
				...
				for( ; phiS != endPhiS; phiS++ ){
					...
					edgesDot << blockLabel << ":" << phiS->id << "->"
						<< blockLabel << FULLGRAPH_PHI << ":" << phiS->id;
					if( divergenceGraph.isDivNode(phiS->id) ){
						file << " BGCOLOR=\"tomato\"";
			...
			file << "</TR><TR>" << secondLine.str()
				<< "</TR><TR><TD COLSPAN=\"" << totalSources
				<< "\" PORT=\"out\">Phis</TD></TR></TABLE>>];";
...
/* Constructor that sets everything and make it ready to draw, sets profiling
	on being able to read the profiling results */
DivergenceDrawer::DivergenceDrawer( const std::string &kernelName,
	const std::string &path, analysis::DivergenceAnalysis *divergenceAnalysis,
		const bool &allGraphs, const bool &divergenceGraph,
		const bool &varsGraph, const bool &dfgGraph, const bool &cfgGraph) :
		_kernelName(kernelName), _path(path), divAnalysis(divergenceAnalysis),
		_all(allGraphs), _divergence(divergenceGraph), _vars(varsGraph),
		_dfg(dfgGraph),
		_cfg(cfgGraph)
{
	if( _all ){
...
/* Constructor that accepts less variables, and sets profiling on being able
	to read the profiling results */
DivergenceDrawer::DivergenceDrawer( const std::string &kernelName,
	const std::string &path, analysis::DivergenceAnalysis *divergenceAnalysis,
	const bool &allGraphs ){
	if( _all ){
		_divergence = true;
		_vars = true;
		_dfg = true;
