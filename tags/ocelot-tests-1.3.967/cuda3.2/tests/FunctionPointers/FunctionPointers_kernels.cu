...
void setupFunctionTables()
{
	...
	cutilSafeCall( cudaMemcpyFromSymbol( &h_blockFunction_table[SOBEL_FILTER], pComputeSobel, sizeof(blockFunction_t)) );
	cutilSafeCall( cudaMemcpyFromSymbol( &h_blockFunction_table[BOX_FILTER], pComputeBox, sizeof(blockFunction_t) ) );

	// do the same for the point function, where the 2nd function is NULL ("no-op" filter, skipped in kernel code)
	cutilSafeCall( cudaMemcpyFromSymbol( &h_pointFunction_table[THRESHOLD_FILTER], pComputeThreshold, sizeof(pointFunction_t) ) );
	...
	cutilSafeCall( cudaMemcpyToSymbol( blockFunction_table, h_blockFunction_table, sizeof( blockFunction_t )*LAST_BLOCK_FILTER ) ); 
	cutilSafeCall( cudaMemcpyToSymbol( pointFunction_table, h_pointFunction_table, sizeof( pointFunction_t )*LAST_POINT_FILTER ) ); 
}
