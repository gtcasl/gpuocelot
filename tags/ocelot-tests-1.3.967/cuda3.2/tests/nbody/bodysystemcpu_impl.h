...
template<typename T>
T sqrt_T(T x) { return sqrt(x); }

template<>
float sqrt_T<float>(float x) { return sqrtf(x); }
...
void bodyBodyInteraction(T accel[3], T posMass0[4], T posMass1[4], T softeningSquared)
{
    ...
    T invDist = (T)1.0 / sqrt((double)distSqr);
...
void BodySystemCPU<T>::_computeNBodyGravitation() 
{
    ...
    for(int i = 0; i < m_numBodies; ++i) 
    {
	    ...
	    while ( j < m_numBodies)
	    {
	        bodyBodyInteraction<T>(acc, &m_pos[i], &m_pos[j], m_softeningSquared);
            j+=4;
            bodyBodyInteraction<T>(acc, &m_pos[i], &m_pos[j], m_softeningSquared);
            j+=4;
            bodyBodyInteraction<T>(acc, &m_pos[i], &m_pos[j], m_softeningSquared);
