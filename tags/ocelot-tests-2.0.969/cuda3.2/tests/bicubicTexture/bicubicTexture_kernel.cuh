...
// version using gather4 texture function
// - this returns the four neighbouring samples in a single texture lookup
// - you can select which component to fetch using the "comp" parameter
// - it is only supported on Fermi
//
// x: (0, 1)
// y: (1, 1)
// z: (1, 0)
...
template<class T, class R>  // return type, texture type
__device__
R tex2DBilinearGather(const texture<T, 2, cudaReadModeNormalizedFloat> tex, float x, float y)
{
    x -= 0.5f;
    y -= 0.5f;
    float px = floorf(x);   // integer position
    ...
    uchar4 samples = tex2Dgather(tex, px + 0.5f, py + 0.5f, 0);

    return lerp( lerp( samples.w, samples.z, fx ),
                 lerp( samples.x, samples.y, fx ), fy );
}
...
d_render(uchar4 *d_output, uint width, uint height, float tx, float ty, float scale, float cx, float cy)
{
    ...
    if ((x < width) && (y < height)) {
        ...
        //float c = tex2DBilinearGather<uchar, float>(tex, u, v);
