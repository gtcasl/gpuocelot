...
int main(int argc, char **argv)
{
    ...
    if( (deviceProp.concurrentKernels == 0 ))
        shrLog("> GPU does not support concurrent kernel execution, kernel runs will be serialized\n");
    ...
    // queue a sum kernel and a copy back to host in the last stream. the commands in this stream get dispatched as soon as all the kernel events have been recorded
    ...
    if( qatest ) 
	shrLog("%s \n\n", (a[0] >  total_clocks) ? "PASSED" : "FAILED");
