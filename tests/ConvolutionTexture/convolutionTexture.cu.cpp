# 1 "/tmp/tmpxft_00000c66_00000000-1_convolutionTexture.cudafe1.cpp"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/tmp/tmpxft_00000c66_00000000-1_convolutionTexture.cudafe1.cpp"
# 1 "convolutionTexture.cu"
...
# 57 "convolutionTexture.cu"
int iDivUp(int a, int b) {
# 58 "convolutionTexture.cu"
return ((a % b) != 0) ? (a / b + 1) : (a / b);
# 59 "convolutionTexture.cu"
}
# 62 "convolutionTexture.cu"
int iAlignUp(int a, int b) {
# 63 "convolutionTexture.cu"
return ((a % b) != 0) ? ((a - a % b) + b) : a;
# 64 "convolutionTexture.cu"
}
# 71 "convolutionTexture.cu"
extern "C" void convolutionRowCPU(float *, float *, float *, int, int, int);
# 80 "convolutionTexture.cu"
...
static float __shadow_d_Kernel[((2 * 8) + 1)];





# 1 "/tmp/tmpxft_00000c66_00000000-1_convolutionTexture.cudafe1.stub.h" 1
...
# 60 "convolutionTexture_kernel.cu" 2
...
# 63 "convolutionTexture_kernel.cu" 2
# 107 "convolutionTexture.cu"
static const int DATA_W = iAlignUp(3072, 16);
# 108 "convolutionTexture.cu"
static const int DATA_H = 3072;
# 111 "convolutionTexture.cu"
static const int KERNEL_SIZE = (((2 * 8) + 1) * sizeof(float));
# 112 "convolutionTexture.cu"
static const int DATA_SIZE = ((DATA_W * DATA_H) * sizeof(float));
# 119 "convolutionTexture.cu"
int main(int argc, char **argv) {
# 121 "convolutionTexture.cu"
auto float *h_Kernel;
# 122 "convolutionTexture.cu"
auto float *h_DataA;
# 123 "convolutionTexture.cu"
auto float *h_DataB;
# 124 "convolutionTexture.cu"
auto float *h_ResultGPU;
# 127 "convolutionTexture.cu"
auto cudaArray *a_Data;
# 129 "convolutionTexture.cu"
auto cudaChannelFormatDesc floatTex = cudaCreateChannelDesc< float> ();
# 132 "convolutionTexture.cu"
auto float *d_Result;
# 135 "convolutionTexture.cu"
auto double gpuTime; auto double L1norm; auto double rCPU; auto double rGPU; auto double sum_delta; auto double sum_ref;
# 137 "convolutionTexture.cu"
auto int i; auto int x; auto int y;
# 139 "convolutionTexture.cu"
auto unsigned hTimer;
# 144 "convolutionTexture.cu"
if (cutCheckCmdLineFlag(argc, (const char **)argv, "device")) {
# 145 "convolutionTexture.cu"
cutilDeviceInit(argc, argv); } else {
# 147 "convolutionTexture.cu"
cudaSetDevice(cutGetMaxGflopsDeviceId()); }
# 149 "convolutionTexture.cu"
__cutilCheckError(cutCreateTimer(&hTimer), "convolutionTexture.cu", 149);
# 151 "convolutionTexture.cu"
printf("Initializing data...\n");
# 152 "convolutionTexture.cu"
h_Kernel = (float *)malloc(KERNEL_SIZE);
# 153 "convolutionTexture.cu"
h_DataA = (float *)malloc(DATA_SIZE);
# 154 "convolutionTexture.cu"
h_DataB = (float *)malloc(DATA_SIZE);
# 155 "convolutionTexture.cu"
h_ResultGPU = (float *)malloc(DATA_SIZE);
# 156 "convolutionTexture.cu"
__cudaSafeCall(cudaMallocArray(&a_Data, &floatTex, DATA_W, DATA_H), "convolutionTexture.cu", 156);
# 157 "convolutionTexture.cu"
__cudaSafeCall(cudaMalloc((void **)(&d_Result), DATA_SIZE), "convolutionTexture.cu", 157);
# 159 "convolutionTexture.cu"
auto float kernelSum = (0);
# 160 "convolutionTexture.cu"
for (i = 0; i < ((2 * 8) + 1); i++) {
# 161 "convolutionTexture.cu"
auto float dist = ((float)(i - 8) / ((float)8));
# 162 "convolutionTexture.cu"
(h_Kernel[i]) = expf((-dist * dist) / (2));
# 163 "convolutionTexture.cu"
kernelSum += h_Kernel[i];
# 164 "convolutionTexture.cu"
}
# 165 "convolutionTexture.cu"
for (i = 0; i < ((2 * 8) + 1); i++) {
# 166 "convolutionTexture.cu"
(h_Kernel[i]) /= kernelSum; }
# 168 "convolutionTexture.cu"
srand(2007);
# 169 "convolutionTexture.cu"
for (i = 0; i < (DATA_W * DATA_H); i++) {
# 170 "convolutionTexture.cu"
(h_DataA[i]) = (float)rand() / ((float)2147483647); }
# 172 "convolutionTexture.cu"
__cudaSafeCall(cudaMemcpyToSymbol(__shadow_d_Kernel, h_Kernel, KERNEL_SIZE), "convolutionTexture.cu", 172);
# 173 "convolutionTexture.cu"
__cudaSafeCall(cudaMemcpyToArray(a_Data, 0, 0, h_DataA, DATA_SIZE, cudaMemcpyHostToDevice), "convolutionTexture.cu", 173);
# 174 "convolutionTexture.cu"
__cudaSafeCall(cudaBindTextureToArray(texData, a_Data), "convolutionTexture.cu", 174);
# 178 "convolutionTexture.cu"
auto dim3 threadBlock(16, 12);
# 179 "convolutionTexture.cu"
auto dim3 blockGrid(iDivUp(DATA_W, threadBlock.x), iDivUp(DATA_H, threadBlock.y));
# 182 "convolutionTexture.cu"
printf("convolutionRowGPU()\n");
# 183 "convolutionTexture.cu"
__cudaSafeCall(cudaThreadSynchronize(), "convolutionTexture.cu", 183);
# 184 "convolutionTexture.cu"
__cutilCheckError(cutResetTimer(hTimer), "convolutionTexture.cu", 184);
# 185 "convolutionTexture.cu"
__cutilCheckError(cutStartTimer(hTimer), "convolutionTexture.cu", 185);
# 186 "convolutionTexture.cu"
cudaConfigureCall(blockGrid, threadBlock) ? ((void)0) : __device_stub__Z17convolutionRowGPUPfii(d_Result, DATA_W, DATA_H);
# 191 "convolutionTexture.cu"
__cutilCheckMsg("convolutionRowGPU() execution failed\n", "convolutionTexture.cu", 191);
# 192 "convolutionTexture.cu"
__cudaSafeCall(cudaThreadSynchronize(), "convolutionTexture.cu", 192);
# 193 "convolutionTexture.cu"
__cutilCheckError(cutStopTimer(hTimer), "convolutionTexture.cu", 193);
# 194 "convolutionTexture.cu"
gpuTime = cutGetTimerValue(hTimer);
# 195 "convolutionTexture.cu"
...
# 199 "convolutionTexture.cu"
printf("Copying convolutionRowGPU() output back to a_Data...\n");
# 200 "convolutionTexture.cu"
__cutilCheckError(cutResetTimer(hTimer), "convolutionTexture.cu", 200);
# 201 "convolutionTexture.cu"
__cutilCheckError(cutStartTimer(hTimer), "convolutionTexture.cu", 201);
# 202 "convolutionTexture.cu"
__cudaSafeCall(cudaMemcpyToArray(a_Data, 0, 0, d_Result, DATA_SIZE, cudaMemcpyDeviceToDevice), "convolutionTexture.cu", 202);
# 203 "convolutionTexture.cu"
__cudaSafeCall(cudaThreadSynchronize(), "convolutionTexture.cu", 203);
# 204 "convolutionTexture.cu"
__cutilCheckError(cutStopTimer(hTimer), "convolutionTexture.cu", 204);
# 205 "convolutionTexture.cu"
gpuTime = cutGetTimerValue(hTimer);
# 206 "convolutionTexture.cu"
printf("...cudaMemcpyToArray() time: %f msecs; //%f Mpix/s\n", gpuTime, ((DATA_W * DATA_H) * (9.999999999999999547e-07)) / ((0.001000000000000000021) * gpuTime));
# 209 "convolutionTexture.cu"
printf("convolutionColumnGPU()...\n");
# 210 "convolutionTexture.cu"
__cutilCheckError(cutResetTimer(hTimer), "convolutionTexture.cu", 210);
# 211 "convolutionTexture.cu"
__cutilCheckError(cutStartTimer(hTimer), "convolutionTexture.cu", 211);
# 212 "convolutionTexture.cu"
cudaConfigureCall(blockGrid, threadBlock) ? ((void)0) : __device_stub__Z20convolutionColumnGPUPfii(d_Result, DATA_W, DATA_H);
# 217 "convolutionTexture.cu"
__cutilCheckMsg("convolutionColumnGPU() execution failed\n", "convolutionTexture.cu", 217);
# 218 "convolutionTexture.cu"
__cudaSafeCall(cudaThreadSynchronize(), "convolutionTexture.cu", 218);
# 219 "convolutionTexture.cu"
__cutilCheckError(cutStopTimer(hTimer), "convolutionTexture.cu", 219);
# 220 "convolutionTexture.cu"
gpuTime = cutGetTimerValue(hTimer);
# 221 "convolutionTexture.cu"
printf("...convolutionColumnGPU() time: %f msecs; //%f Mpix/s\n", gpuTime, ((DATA_W * DATA_H) * (9.999999999999999547e-07)) / ((0.001000000000000000021) * gpuTime));
# 224 "convolutionTexture.cu"
printf("Reading back GPU results...\n");
# 225 "convolutionTexture.cu"
__cudaSafeCall(cudaMemcpy(h_ResultGPU, d_Result, DATA_SIZE, cudaMemcpyDeviceToHost), "convolutionTexture.cu", 225);
# 228 "convolutionTexture.cu"
printf("Checking GPU results...\n");
# 229 "convolutionTexture.cu"
printf("...convolutionRowCPU()\n");
# 230 "convolutionTexture.cu"
convolutionRowCPU(h_DataB, h_DataA, h_Kernel, DATA_W, DATA_H, 8);
# 239 "convolutionTexture.cu"
printf("...convolutionColumnCPU()\n");
# 240 "convolutionTexture.cu"
convolutionColumnCPU(h_DataA, h_DataB, h_Kernel, DATA_W, DATA_H, 8);
# 249 "convolutionTexture.cu"
printf("...comparing the results\n");
# 250 "convolutionTexture.cu"
sum_delta = (0);
# 251 "convolutionTexture.cu"
sum_ref = (0);
# 252 "convolutionTexture.cu"
for (y = 0; y < DATA_H; y++) {
# 253 "convolutionTexture.cu"
for (x = 0; x < DATA_W; x++) {
# 254 "convolutionTexture.cu"
rCPU = h_DataA[y * DATA_W + x];
# 255 "convolutionTexture.cu"
rGPU = h_ResultGPU[y * DATA_W + x];
# 256 "convolutionTexture.cu"
sum_delta += fabs(rCPU - rGPU);
# 257 "convolutionTexture.cu"
sum_ref += fabs(rCPU);
# 258 "convolutionTexture.cu"
} }
# 259 "convolutionTexture.cu"
L1norm = sum_delta / sum_ref;
# 260 "convolutionTexture.cu"
printf("L1 norm: %E\n", L1norm);
# 261 "convolutionTexture.cu"
printf((L1norm < (9.999999999999999547e-07)) ? ((const char *)"TEST PASSED\n") : ((const char *)"TEST FAILED\n"));
# 264 "convolutionTexture.cu"
printf("Shutting down...\n");
# 265 "convolutionTexture.cu"
__cudaSafeCall(cudaUnbindTexture(texData), "convolutionTexture.cu", 265);
# 266 "convolutionTexture.cu"
__cudaSafeCall(cudaFree(d_Result), "convolutionTexture.cu", 266);
# 267 "convolutionTexture.cu"
__cudaSafeCall(cudaFreeArray(a_Data), "convolutionTexture.cu", 267);
# 268 "convolutionTexture.cu"
free(h_ResultGPU);
# 269 "convolutionTexture.cu"
free(h_DataB);
# 270 "convolutionTexture.cu"
free(h_DataA);
# 271 "convolutionTexture.cu"
free(h_Kernel);
# 273 "convolutionTexture.cu"
__cutilExit(argc, argv);
# 275 "convolutionTexture.cu"
cudaThreadExit(); return 0;
# 276 "convolutionTexture.cu"
}

# 1 "/tmp/tmpxft_00000c66_00000000-1_convolutionTexture.cudafe1.stub.c" 1

extern "C" {

# 1 "/tmp/tmpxft_00000c66_00000000-3_convolutionTexture.fatbin.c" 1
...
# 2 "/tmp/tmpxft_00000c66_00000000-3_convolutionTexture.fatbin.c" 2
...
0x2020676572090a32ull,0x6162090a3231203dull,0x090a30203d202072ull,0x0a7b2074736e6f63ull,
...
0x0a7b206d656d0909ull,0x3030307830090909ull,0x090a206666333030ull,0x62090a7d090a7d09ull,
0x7b2065646f636e69ull,0x303061783009090aull,0x7830206430343430ull,0x3038373030323430ull,
0x3430303061783020ull,0x3430783020313165ull,0x0a20303837303032ull,0x3038306478300909ull,
0x3078302035303230ull,0x2030383730303430ull,0x3234303030617830ull,0x3234307830203530ull,
0x090a203038373030ull,0x3430303061783009ull,0x3430783020393063ull,0x3020303837303032ull,
0x3036303430303478ull,0x3130303078302064ull,0x09090a2030383738ull,0x3230303030617830ull,
0x3034307830203131ull,0x7830203038373030ull,0x3930323032303034ull,0x3831303030783020ull,
0x3009090a20303837ull,0x3030303030306178ull,0x3030343078302031ull,0x3278302030383730ull,
0x2034303638343030ull,0x3438303030327830ull,0x783009090a203030ull,0x6466666331303033ull,
0x3031326336783020ull,0x3033783020386337ull,0x3020646664633030ull,0x6332303132633678ull,
0x33783009090a2038ull,0x2033303030303030ull,0x3130303030307830ull,0x3030617830203030ull,
0x7830203930303030ull,0x3038373431303434ull,0x3061783009090a20ull,0x3020643032303030ull,
0x3837343130343478ull,0x3030306278302030ull,0x3078302039303430ull,0x2033303030306633ull,
0x303062783009090aull,0x7830203532363030ull,0x3330303030663330ull,0x3030303062783020ull,
0x3430783020393234ull,0x0a20333030303031ull,0x3030303178300909ull,0x3078302064313231ull,
0x2030383763333036ull,0x3431303030627830ull,0x6662307830203931ull,0x090a203330303038ull,
0x6331303036783009ull,0x3038783020313263ull,0x3020303837303032ull,0x3130303034326678ull,
0x3030303078302039ull,0x09090a2034383730ull,0x3239303030317830ull,0x3030317830206330ull,
0x7830203430323932ull,0x3431323930303031ull,0x3031383063783020ull,0x3009090a20633263ull,
static __cudaFatCudaBinary __fatDeviceText __attribute__ ((section (".nvFatBinSegment")))= {0x1ee55a01,0x00000003,0x8ecc680c,(char*)"6a12cad95b54c4ee",(char*)"convolutionTexture.cu",(char*)" ",__ptxEntries,__cubinEntries,__debugEntries,0,0,0,0,0};
# 5 "/tmp/tmpxft_00000c66_00000000-1_convolutionTexture.cudafe1.stub.c" 2
...
# 6 "/tmp/tmpxft_00000c66_00000000-1_convolutionTexture.cudafe1.stub.c" 2
struct __T20;
struct __T21;
struct __T20 {float *__par0;int __par1;int __par2;int __dummy_field;volatile char __dummy[4];};
struct __T21 {float *__par0;int __par1;int __par2;int __dummy_field;volatile char __dummy[4];};
...
static void __sti____cudaRegisterAll_53_tmpxft_00000c66_00000000_7_convolutionTexture_cpp1_ii_c14de7b3(void) __attribute__((__constructor__));
void __device_stub__Z17convolutionRowGPUPfii(float *__par0, int __par1, int __par2){auto struct __T20 *__T22;
char __[256]; *(char**)&__T22 = __;if (cudaSetupArgument((void*)(char*)&__par0, sizeof(__par0), (size_t)&__T22->__par0 - (size_t)__T22) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par1, sizeof(__par1), (size_t)&__T22->__par1 - (size_t)__T22) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par2, sizeof(__par2), (size_t)&__T22->__par2 - (size_t)__T22) != cudaSuccess) return;{ volatile static char *__f; __f = ((char *)__device_stub__Z17convolutionRowGPUPfii); (void)cudaLaunch(((char *)__device_stub__Z17convolutionRowGPUPfii)); };}
void __device_stub__Z20convolutionColumnGPUPfii(float *__par0, int __par1, int __par2){auto struct __T21 *__T27;
char __[256]; *(char**)&__T27 = __;if (cudaSetupArgument((void*)(char*)&__par0, sizeof(__par0), (size_t)&__T27->__par0 - (size_t)__T27) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par1, sizeof(__par1), (size_t)&__T27->__par1 - (size_t)__T27) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par2, sizeof(__par2), (size_t)&__T27->__par2 - (size_t)__T27) != cudaSuccess) return;{ volatile static char *__f; __f = ((char *)__device_stub__Z20convolutionColumnGPUPfii); (void)cudaLaunch(((char *)__device_stub__Z20convolutionColumnGPUPfii)); };}
...
static void __sti____cudaRegisterAll_53_tmpxft_00000c66_00000000_7_convolutionTexture_cpp1_ii_c14de7b3(void){__cudaFatCubinHandle = __cudaRegisterFatBinary((void*)(&__fatDeviceText)); atexit(__cudaUnregisterBinaryUtil);__cudaRegisterFunction(__cudaFatCubinHandle, (const char*)__device_stub__Z20convolutionColumnGPUPfii, (char*)"_Z20convolutionColumnGPUPfii", "_Z20convolutionColumnGPUPfii", (-1), (uint3*)0, (uint3*)0, (dim3*)0, (dim3*)0, (int*)0);__cudaRegisterFunction(__cudaFatCubinHandle, (const char*)__device_stub__Z17convolutionRowGPUPfii, (char*)"_Z17convolutionRowGPUPfii", "_Z17convolutionRowGPUPfii", (-1), (uint3*)0, (uint3*)0, (dim3*)0, (dim3*)0, (int*)0);__cudaRegisterTexture(__cudaFatCubinHandle, (const struct textureReference*)&texData, 0, "texData", 2, 0, 0);__cudaRegisterVar(__cudaFatCubinHandle, (char*)&__shadow_d_Kernel, (char*)"d_Kernel", "d_Kernel", 0, 68, 1, 0);}

}
# 278 "convolutionTexture.cu" 2
