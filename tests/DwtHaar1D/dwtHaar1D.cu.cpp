# 1 "/tmp/tmpxft_00000dd6_00000000-1_dwtHaar1D.cudafe1.cpp"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/tmp/tmpxft_00000dd6_00000000-1_dwtHaar1D.cudafe1.cpp"
# 1 "dwtHaar1D.cu"
...
# 1 "/tmp/tmpxft_00000dd6_00000000-1_dwtHaar1D.cudafe1.stub.h" 1
...
# 115 "dwtHaar1D.cu"
extern CUTBoolean getLevels(unsigned, unsigned *);
# 121 "dwtHaar1D.cu"
...
# 178 "dwtHaar1D.cu"
auto unsigned slength = (0);
# 179 "dwtHaar1D.cu"
auto float *signal = (__null);
# 180 "dwtHaar1D.cu"
if (s_fname == (0))
# 181 "dwtHaar1D.cu"
{
...
# 194 "dwtHaar1D.cu"
auto unsigned dlevels_complete = (0);
# 195 "dwtHaar1D.cu"
...
# 209 "dwtHaar1D.cu"
auto float *approx_final = (__null);
# 215 "dwtHaar1D.cu"
auto int approx_is_input;
# 218 "dwtHaar1D.cu"
auto const unsigned smem_size = (sizeof(float) * slength);
# 219 "dwtHaar1D.cu"
__cudaSafeCall(cudaMalloc((void **)(&d_idata), smem_size), "dwtHaar1D.cu", 219);
# 220 "dwtHaar1D.cu"
__cudaSafeCall(cudaMalloc((void **)(&d_odata), smem_size), "dwtHaar1D.cu", 220);
# 221 "dwtHaar1D.cu"
__cudaSafeCall(cudaMalloc((void **)(&approx_final), smem_size), "dwtHaar1D.cu", 221);
# 223 "dwtHaar1D.cu"
__cudaSafeCall(cudaMemcpy(d_idata, signal, smem_size, cudaMemcpyHostToDevice), "dwtHaar1D.cu", 224);
# 227 "dwtHaar1D.cu"
auto float *tmp = ((float *)malloc(smem_size));
# 228 "dwtHaar1D.cu"
...
# 232 "dwtHaar1D.cu"
__cudaSafeCall(cudaMemcpy(d_odata, tmp, smem_size, cudaMemcpyHostToDevice), "dwtHaar1D.cu", 233);
# 234 "dwtHaar1D.cu"
free(tmp);
# 239 "dwtHaar1D.cu"
auto unsigned num_threads_total_left = (slength / (2));
# 241 "dwtHaar1D.cu"
auto unsigned dlevels_step = dlevels_complete;
# 244 "dwtHaar1D.cu"
auto dim3 block_size;
# 245 "dwtHaar1D.cu"
auto dim3 grid_size;
# 248 "dwtHaar1D.cu"
auto unsigned dlevels_left = dlevels_complete;
# 253 "dwtHaar1D.cu"
...
{
# 276 "dwtHaar1D.cu"
auto unsigned mem_shared = (((2) * block_size.x) * sizeof(float));
# 278 "dwtHaar1D.cu"
mem_shared += (((2) * block_size.x) / NUM_BANKS) * sizeof(float);
# 281 "dwtHaar1D.cu"
cudaConfigureCall(grid_size, block_size, mem_shared) ? ((void)0) : __device_stub__Z9dwtHaar1DPfS_S_jji(d_idata, d_odata, approx_final, dlevels_step, num_threads_total_left, block_size.x);
# 288 "dwtHaar1D.cu"
if (approx_is_input)
# 289 "dwtHaar1D.cu"
{
# 290 "dwtHaar1D.cu"
__cudaSafeCall(cudaMemcpy(d_idata, approx_final, grid_size.x * (4), cudaMemcpyDeviceToDevice), "dwtHaar1D.cu", 291);
# 292 "dwtHaar1D.cu"
} else
# 294 "dwtHaar1D.cu"
{
# 295 "dwtHaar1D.cu"
__cudaSafeCall(cudaMemcpy(d_odata, approx_final, grid_size.x * (4), cudaMemcpyDeviceToDevice), "dwtHaar1D.cu", 296);
# 297 "dwtHaar1D.cu"
}
# 300 "dwtHaar1D.cu"
if (dlevels_left < (10))
...
# 325 "dwtHaar1D.cu"
auto float *odata = ((float *)malloc(smem_size));
# 326 "dwtHaar1D.cu"
__cudaSafeCall(cudaMemcpy(odata, d_odata, smem_size, cudaMemcpyDeviceToHost), "dwtHaar1D.cu", 327);
# 330 "dwtHaar1D.cu"
...
# 362 "dwtHaar1D.cu"
__cutilCondition(slength == len_reference, (char *)("dwtHaar1D.cu"), 362);
# 365 "dwtHaar1D.cu"
auto CUTBoolean res = cutComparefe(reference, odata, slength, (0.001000000047F));
# 366 "dwtHaar1D.cu"
...
# 371 "dwtHaar1D.cu"
__cudaSafeCall(cudaFree(d_odata), "dwtHaar1D.cu", 371);
# 372 "dwtHaar1D.cu"
__cudaSafeCall(cudaFree(d_idata), "dwtHaar1D.cu", 372);
# 373 "dwtHaar1D.cu"
__cudaSafeCall(cudaFree(approx_final), "dwtHaar1D.cu", 373);
# 375 "dwtHaar1D.cu"
cutFree(signal);
# 376 "dwtHaar1D.cu"
free(odata);
...
{
# 399 "dwtHaar1D.cu"
auto CUTBoolean retval = CUTFalse;
# 402 "dwtHaar1D.cu"
...
# 1 "/tmp/tmpxft_00000dd6_00000000-1_dwtHaar1D.cudafe1.stub.c" 1

extern "C" {

# 1 "/tmp/tmpxft_00000dd6_00000000-3_dwtHaar1D.fatbin.c" 1
...
# 2 "/tmp/tmpxft_00000dd6_00000000-3_dwtHaar1D.fatbin.c" 2
...
0x20656d616e090a7bull,0x747764395a5f203dull,0x6650443172616148ull,0x0a696a6a5f535f53ull,
0x203d206d656d6c09ull,0x206d656d73090a30ull,0x6572090a3436203dull,0x0a3131203d202067ull,
...
0x6430633430303061ull,0x3030323430783020ull,0x3031783020303837ull,0x3020353038313030ull,
0x3837633030343478ull,0x34783009090a2030ull,0x2039303430373030ull,0x3730303030307830ull,
0x3030367830203038ull,0x7830203930363036ull,0x3038373830303030ull,0x3033783009090a20ull,
0x3020393034303031ull,0x3837303031346378ull,0x3630303678302030ull,0x3078302031313430ull,
0x2030383738303030ull,0x303061783009090aull,0x7830203531303030ull,0x3038373030303430ull,
0x3031303033783020ull,0x3463783020313038ull,0x0a20303837303031ull,0x3030303278300909ull,
0x3278302030306138ull,0x2034303866303031ull,0x3030323030337830ull,0x3134637830203130ull,
0x090a203038373030ull,0x3032303033783009ull,0x3463783020353032ull,0x3020303837303031ull,
0x3038633030303278ull,0x3032343078302031ull,0x09090a2030383730ull,0x6130323030307830ull,
0x36752e646461090aull,0x3733647225092034ull,0x2c3433647225202cull,0x203b363364722520ull,
0x7473090a202f2f09ull,0x2e6c61626f6c672eull,0x72255b0920323366ull,0x202c5d302b373364ull,
0x2f2f093b31326625ull,0x0a3532313a646920ull,0x32325f305f744c24ull,0x5f305f744c240a3aull,
0x6f6c2e090a3a3431ull,0x3133320935310963ull,0x74697865090a3009ull,0x202020202020203bull,
0x2020202020202020ull,0x2020202020202020ull,0x240a202f2f092020ull,0x5f5f646e6557444cull,
0x616148747764395aull,0x535f536650443172ull,0x7d090a3a696a6a5full,0x64395a5f202f2f20ull,
0x4431726161487477ull,0x6a6a5f535f536650ull,0x00000000000a0a69ull
};
...
static __cudaFatCudaBinary __fatDeviceText __attribute__ ((section (".nvFatBinSegment")))= {0x1ee55a01,0x00000003,0x8ecc680c,(char*)"1b9d13bfac599104",(char*)"dwtHaar1D.cu",(char*)" ",__ptxEntries,__cubinEntries,__debugEntries,0,0,0,0,0};
# 5 "/tmp/tmpxft_00000dd6_00000000-1_dwtHaar1D.cudafe1.stub.c" 2
...
# 6 "/tmp/tmpxft_00000dd6_00000000-1_dwtHaar1D.cudafe1.stub.c" 2
struct __T20;
struct __T20 {float *__par0;float *__par1;float *__par2;unsigned __par3;unsigned __par4;int __par5;int __dummy_field;};



static void __sti____cudaRegisterAll_44_tmpxft_00000dd6_00000000_7_dwtHaar1D_cpp1_ii_c14de7b3(void) __attribute__((__constructor__));
void __device_stub__Z9dwtHaar1DPfS_S_jji(float *__par0, float *__par1, float *__par2, const unsigned __par3, const unsigned __par4, const int __par5){auto struct __T20 *__T21;
char __[256]; *(char**)&__T21 = __;if (cudaSetupArgument((void*)(char*)&__par0, sizeof(__par0), (size_t)&__T21->__par0 - (size_t)__T21) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par1, sizeof(__par1), (size_t)&__T21->__par1 - (size_t)__T21) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par2, sizeof(__par2), (size_t)&__T21->__par2 - (size_t)__T21) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par3, sizeof(__par3), (size_t)&__T21->__par3 - (size_t)__T21) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par4, sizeof(__par4), (size_t)&__T21->__par4 - (size_t)__T21) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par5, sizeof(__par5), (size_t)&__T21->__par5 - (size_t)__T21) != cudaSuccess) return;{ volatile static char *__f; __f = ((char *)__device_stub__Z9dwtHaar1DPfS_S_jji); (void)cudaLaunch(((char *)__device_stub__Z9dwtHaar1DPfS_S_jji)); };}



static void __sti____cudaRegisterAll_44_tmpxft_00000dd6_00000000_7_dwtHaar1D_cpp1_ii_c14de7b3(void){__cudaFatCubinHandle = __cudaRegisterFatBinary((void*)(&__fatDeviceText)); atexit(__cudaUnregisterBinaryUtil);__cudaRegisterFunction(__cudaFatCubinHandle, (const char*)__device_stub__Z9dwtHaar1DPfS_S_jji, (char*)"_Z9dwtHaar1DPfS_S_jji", "_Z9dwtHaar1DPfS_S_jji", (-1), (uint3*)0, (uint3*)0, (dim3*)0, (dim3*)0, (int*)0);__cudaRegisterShared(__cudaFatCubinHandle, (void**)"shared");}
