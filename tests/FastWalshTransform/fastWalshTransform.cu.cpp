# 1 "/tmp/tmpxft_00001050_00000000-1_fastWalshTransform.cudafe1.cpp"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/tmp/tmpxft_00001050_00000000-1_fastWalshTransform.cudafe1.cpp"
# 1 "fastWalshTransform.cu"
...
# 61 "fastWalshTransform.cu"
extern "C" void fwtCPU(float *, float *, int);
# 62 "fastWalshTransform.cu"
extern "C" void slowWTcpu(float *, float *, int);
# 63 "fastWalshTransform.cu"
extern "C" void dyadicConvolutionCPU(float *, float *, float *, int, int);

# 1 "/tmp/tmpxft_00001050_00000000-1_fastWalshTransform.cudafe1.stub.h" 1

extern "C" {


extern void __device_stub__Z15fwtBatch1KernelPfS_i(float *, float *, int);
# 14 "/tmp/tmpxft_00001050_00000000-1_fastWalshTransform.cudafe1.stub.h"
}
# 66 "fastWalshTransform.cu" 2
...
# 69 "fastWalshTransform.cu" 2
# 140 "fastWalshTransform_kernel.cu"
void fwtBatchGPU(float *d_Data, int M, int log2N) {
# 141 "fastWalshTransform_kernel.cu"
auto const int THREAD_N = 256;
# 143 "fastWalshTransform_kernel.cu"
auto int N = (1 << log2N);
# 144 "fastWalshTransform_kernel.cu"
auto dim3 grid((1 << log2N) / (4 * THREAD_N), M, 1);
# 145 "fastWalshTransform_kernel.cu"
for (; log2N > 11; ((log2N -= 2), (N >>= 2)), (M <<= 2)) {
# 146 "fastWalshTransform_kernel.cu"
cudaConfigureCall(grid, THREAD_N) ? ((void)0) : __device_stub__Z15fwtBatch2KernelPfS_i(d_Data, d_Data, N / 4);
# 147 "fastWalshTransform_kernel.cu"
__cutilCheckMsg("fwtBatch2Kernel() execution failed\n", "fastWalshTransform_kernel.cu", 147);
# 148 "fastWalshTransform_kernel.cu"
}
# 150 "fastWalshTransform_kernel.cu"
cudaConfigureCall(M, N / 4, N * sizeof(float)) ? ((void)0) : __device_stub__Z15fwtBatch1KernelPfS_i(d_Data, d_Data, log2N);
# 155 "fastWalshTransform_kernel.cu"
__cutilCheckMsg("fwtBatch1Kernel() execution failed\n", "fastWalshTransform_kernel.cu", 155);
# 156 "fastWalshTransform_kernel.cu"
}

# 1 "/tmp/tmpxft_00001050_00000000-1_fastWalshTransform.cudafe1.stub.h" 1

extern "C" {
# 11 "/tmp/tmpxft_00001050_00000000-1_fastWalshTransform.cudafe1.stub.h"
extern void __device_stub__Z14modulateKernelPfS_i(float *, float *, int);


}
# 159 "fastWalshTransform_kernel.cu" 2
# 173 "fastWalshTransform_kernel.cu"
void modulateGPU(float *d_A, float *d_B, int N) {
# 174 "fastWalshTransform_kernel.cu"
cudaConfigureCall(128, 256) ? ((void)0) : __device_stub__Z14modulateKernelPfS_i(d_A, d_B, N);
# 175 "fastWalshTransform_kernel.cu"
}
# 82 "fastWalshTransform.cu"
static const int log2Kernel = 7;
# 85 "fastWalshTransform.cu"
static const int log2Data = 23;
# 89 "fastWalshTransform.cu"
static const int dataN = (1 << log2Data);
# 90 "fastWalshTransform.cu"
static const int kernelN = (1 << log2Kernel);
# 92 "fastWalshTransform.cu"
static const int DATA_SIZE = ((dataN) * sizeof(float));
# 93 "fastWalshTransform.cu"
static const int KERNEL_SIZE = ((kernelN) * sizeof(float));
# 95 "fastWalshTransform.cu"
static const double NOPS = ((((3.0) * ((double)dataN)) * ((double)log2Data)) / (2.0));
# 102 "fastWalshTransform.cu"
int main(int argc, char *argv[]) {
# 104 "fastWalshTransform.cu"
auto float *h_Data;
# 105 "fastWalshTransform.cu"
auto float *h_Kernel;
# 106 "fastWalshTransform.cu"
auto float *h_ResultCPU;
# 107 "fastWalshTransform.cu"
auto float *h_ResultGPU;
# 110 "fastWalshTransform.cu"
auto float *d_Data;
# 111 "fastWalshTransform.cu"
auto float *d_Kernel;
# 114 "fastWalshTransform.cu"
auto double delta; auto double ref; auto double sum_delta2; auto double sum_ref2; auto double L2norm; auto double gpuTime;
# 116 "fastWalshTransform.cu"
auto unsigned hTimer;
# 117 "fastWalshTransform.cu"
auto int i;
# 120 "fastWalshTransform.cu"
if (cutCheckCmdLineFlag(argc, (const char **)argv, "device")) {
# 121 "fastWalshTransform.cu"
cutilDeviceInit(argc, argv); } else {
# 123 "fastWalshTransform.cu"
cudaSetDevice(cutGetMaxGflopsDeviceId()); }
# 125 "fastWalshTransform.cu"
__cutilCheckError(cutCreateTimer(&hTimer), "fastWalshTransform.cu", 125);
# 127 "fastWalshTransform.cu"
printf("Initializing data...\n");
# 128 "fastWalshTransform.cu"
printf("...allocating CPU memory\n");
# 129 "fastWalshTransform.cu"
__cutilSafeMalloc(h_Kernel = (float *)malloc(KERNEL_SIZE), "fastWalshTransform.cu", 129);
# 130 "fastWalshTransform.cu"
__cutilSafeMalloc(h_Data = (float *)malloc(DATA_SIZE), "fastWalshTransform.cu", 130);
# 131 "fastWalshTransform.cu"
__cutilSafeMalloc(h_ResultCPU = (float *)malloc(DATA_SIZE), "fastWalshTransform.cu", 131);
# 132 "fastWalshTransform.cu"
__cutilSafeMalloc(h_ResultGPU = (float *)malloc(DATA_SIZE), "fastWalshTransform.cu", 132);
# 133 "fastWalshTransform.cu"
printf("...allocating GPU memory\n");
# 134 "fastWalshTransform.cu"
__cudaSafeCall(cudaMalloc((void **)(&d_Kernel), DATA_SIZE), "fastWalshTransform.cu", 134);
# 135 "fastWalshTransform.cu"
__cudaSafeCall(cudaMalloc((void **)(&d_Data), DATA_SIZE), "fastWalshTransform.cu", 135);
# 137 "fastWalshTransform.cu"
printf("...generating data\n");
# 138 "fastWalshTransform.cu"
printf("Data length: %i; kernel length: %i\n", dataN, kernelN);
# 139 "fastWalshTransform.cu"
srand(2007);
# 140 "fastWalshTransform.cu"
for (i = 0; i < kernelN; i++) {
# 141 "fastWalshTransform.cu"
(h_Kernel[i]) = (float)rand() / ((float)2147483647); }
# 143 "fastWalshTransform.cu"
for (i = 0; i < dataN; i++) {
# 144 "fastWalshTransform.cu"
(h_Data[i]) = (float)rand() / ((float)2147483647); }
# 146 "fastWalshTransform.cu"
__cudaSafeCall(cudaMemset(d_Kernel, 0, DATA_SIZE), "fastWalshTransform.cu", 146);
# 147 "fastWalshTransform.cu"
__cudaSafeCall(cudaMemcpy(d_Kernel, h_Kernel, KERNEL_SIZE, cudaMemcpyHostToDevice), "fastWalshTransform.cu", 147);
# 148 "fastWalshTransform.cu"
__cudaSafeCall(cudaMemcpy(d_Data, h_Data, DATA_SIZE, cudaMemcpyHostToDevice), "fastWalshTransform.cu", 148);
# 150 "fastWalshTransform.cu"
printf("Running GPU dyadic convolution using Fast Walsh Transform...\n");
# 151 "fastWalshTransform.cu"
__cudaSafeCall(cudaThreadSynchronize(), "fastWalshTransform.cu", 151);
# 152 "fastWalshTransform.cu"
__cutilCheckError(cutResetTimer(hTimer), "fastWalshTransform.cu", 152);
# 153 "fastWalshTransform.cu"
__cutilCheckError(cutStartTimer(hTimer), "fastWalshTransform.cu", 153);
# 154 "fastWalshTransform.cu"
fwtBatchGPU(d_Data, 1, log2Data);
# 155 "fastWalshTransform.cu"
fwtBatchGPU(d_Kernel, 1, log2Data);
# 156 "fastWalshTransform.cu"
modulateGPU(d_Data, d_Kernel, dataN);
# 157 "fastWalshTransform.cu"
fwtBatchGPU(d_Data, 1, log2Data);
# 158 "fastWalshTransform.cu"
__cudaSafeCall(cudaThreadSynchronize(), "fastWalshTransform.cu", 158);
# 159 "fastWalshTransform.cu"
__cutilCheckError(cutStopTimer(hTimer), "fastWalshTransform.cu", 159);
# 160 "fastWalshTransform.cu"
gpuTime = cutGetTimerValue(hTimer);
# 161 "fastWalshTransform.cu"
printf("GPU time: %f ms; GOP/s: %f\n", gpuTime, NOPS / ((gpuTime * (0.001000000000000000021)) * (1000000000.0)));
# 163 "fastWalshTransform.cu"
printf("Reading back GPU results...\n");
# 164 "fastWalshTransform.cu"
__cudaSafeCall(cudaMemcpy(h_ResultGPU, d_Data, DATA_SIZE, cudaMemcpyDeviceToHost), "fastWalshTransform.cu", 164);
# 166 "fastWalshTransform.cu"
printf("Running straightforward CPU dyadic convolution...\n");
# 167 "fastWalshTransform.cu"
dyadicConvolutionCPU(h_ResultCPU, h_Data, h_Kernel, log2Data, log2Kernel);
# 169 "fastWalshTransform.cu"
printf("Comparing the results...\n");
# 170 "fastWalshTransform.cu"
sum_delta2 = (0);
# 171 "fastWalshTransform.cu"
sum_ref2 = (0);
# 172 "fastWalshTransform.cu"
for (i = 0; i < dataN; i++) {
# 173 "fastWalshTransform.cu"
delta = h_ResultCPU[i] - h_ResultGPU[i];
# 174 "fastWalshTransform.cu"
ref = h_ResultCPU[i];
# 175 "fastWalshTransform.cu"
sum_delta2 += delta * delta;
# 176 "fastWalshTransform.cu"
sum_ref2 += ref * ref;
# 177 "fastWalshTransform.cu"
}
# 178 "fastWalshTransform.cu"
L2norm = sqrt(sum_delta2 / sum_ref2);
# 179 "fastWalshTransform.cu"
printf("L2 norm: %E\n", L2norm);
# 180 "fastWalshTransform.cu"
printf((L2norm < (9.999999999999999547e-07)) ? ((const char *)"TEST PASSED\n") : ((const char *)"TEST FAILED\n"));
# 183 "fastWalshTransform.cu"
printf("Shutting down...\n");
# 184 "fastWalshTransform.cu"
__cutilCheckError(cutDeleteTimer(hTimer), "fastWalshTransform.cu", 184);
# 185 "fastWalshTransform.cu"
__cudaSafeCall(cudaFree(d_Data), "fastWalshTransform.cu", 185);
# 186 "fastWalshTransform.cu"
__cudaSafeCall(cudaFree(d_Kernel), "fastWalshTransform.cu", 186);
# 187 "fastWalshTransform.cu"
free(h_ResultGPU);
# 188 "fastWalshTransform.cu"
free(h_ResultCPU);
# 189 "fastWalshTransform.cu"
free(h_Data);
# 190 "fastWalshTransform.cu"
free(h_Kernel);
# 192 "fastWalshTransform.cu"
cudaThreadExit();
# 194 "fastWalshTransform.cu"
__cutilExit(argc, argv); return 0;
# 195 "fastWalshTransform.cu"
}

# 1 "/tmp/tmpxft_00001050_00000000-1_fastWalshTransform.cudafe1.stub.c" 1

extern "C" {

# 1 "/tmp/tmpxft_00001050_00000000-3_fastWalshTransform.fatbin.c" 1
...
# 2 "/tmp/tmpxft_00001050_00000000-3_fastWalshTransform.fatbin.c" 2
...
0x20656d616e090a7bull,0x776635315a5f203dull,0x4b31686374614274ull,0x5366506c656e7265ull,
0x6d656d6c090a695full,0x6d73090a30203d20ull,0x0a3834203d206d65ull,0x203d202067657209ull,
0x2020726162090a38ull,0x6e6f63090a31203dull,0x0909090a7b207473ull,0x20656d616e676573ull,
...
0x3030303030307830ull,0x0a7d09090a203130ull,0x636e6962090a7d09ull,0x09090a7b2065646full,
0x3038313030317830ull,0x3030307830203930ull,0x7830203330303030ull,0x3530303130303031ull,
0x6330303434783020ull,0x3009090a20303837ull,0x3134303130303378ull,0x3030346378302031ull,
0x6178302030383730ull,0x2035303030303030ull,0x3730303034307830ull,0x783009090a203038ull,
0x3130383031303033ull,0x3031306336783020ull,0x3061783020306437ull,0x3020646631303030ull,
0x6337343130633078ull,0x61783009090a2038ull,0x2033303061313030ull,0x3030303030307830ull,
0x3030317830203030ull,0x7830203130323030ull,0x3038376333303630ull,0x3031783009090a20ull,
0x3020333030613130ull,0x3031313030303078ull,0x3030306178302030ull,0x3078302064306334ull,
0x2030383730303234ull,0x303031783009090aull,0x7830203930303130ull,0x3038376330303434ull,
0x3032303033783020ull,0x3463783020393036ull,0x0a20303837303030ull,0x3230303378300909ull,
0x6378302039303430ull,0x2030383730303134ull,0x3234303030617830ull,0x3234307830203931ull,
0x090a203038373030ull,0x3032303033783009ull,0x3463783020643030ull,0x3020303837303031ull,
0x3063633030303278ull,0x3032343078302039ull,0x09090a2030383738ull,0x6330323030337830ull,
0x3134637830203531ull,0x7830203038373030ull,0x3930363030303032ull,0x3830303430783020ull,
0x3009090a20303837ull,0x3036383033303278ull,0x3030303078302064ull,0x3078302033303030ull,
0x2035303630303030ull,0x3730303030637830ull,0x783009090a203038ull,0x6431343065303064ull,
0x3030633038783020ull,0x3032783020303837ull,0x3020353032303030ull,0x3837383130343078ull,
0x33783009090a2030ull,0x2064663930313030ull,0x3730313063367830ull,0x3034307830203864ull,
0x7830203130303030ull,0x3038376331323465ull,0x3032783009090a20ull,0x3020633061383330ull,
0x3034383530303278ull,0x3130303178302038ull,0x3078302033303033ull,0x2030383231303030ull,
0x303033783009090aull,0x7830203531383032ull,0x3238373030316365ull,0x3063373033783020ull,
0x6336783020646662ull,0x0a20386437633030ull,0x3330303178300909ull,0x3078302033303038ull,
...
0x203b34202c316472ull,0x0a202f2f09202020ull,0x3233662e70637209ull,0x25202c3266250920ull,
0x20202020203b3166ull,0x0920202020202020ull,0x646461090a202f2full,0x722509203233732eull,
...
0x74616c75646f6d34ull,0x506c656e72654b65ull,0x415f645f695f5366ull,0x6469202f2f093b5dull,
0x75635f5f2034333aull,0x5f5f6d7261706164ull,0x6c75646f6d34315aull,0x656e72654b657461ull,
0x645f695f5366506cull,0x090a3078302b415full,0x203436752e646461ull,0x25202c3464722509ull,
0x647225202c336472ull,0x2f09202020203b32ull,0x702e646c090a202full,0x3436752e6d617261ull,
0x202c356472250920ull,0x70616475635f5f5bull,0x34315a5f5f6d7261ull,0x6574616c75646f6dull,
0x66506c656e72654bull,0x5d425f645f695f53ull,0x3a6469202f2f093bull,0x6475635f5f203533ull,
0x5a5f5f6d72617061ull,0x616c75646f6d3431ull,0x6c656e72654b6574ull,0x5f645f695f536650ull,
0x61090a3078302b42ull,0x09203436752e6464ull,0x7225202c36647225ull,0x32647225202c3564ull,
...
0x34202c3764722520ull,0x2f2f09202020203bull,0x5f325f744c240a20ull,0x6c3c2f2f200a3a37ull,
...
0x6e776f6e6b6e7520ull,0x3109636f6c2e090aull,0x0a30093936310935ull,0x626f6c672e646c09ull,
0x09203233662e6c61ull,0x72255b202c336625ull,0x09203b5d302b3464ull,0x36333a6469202f2full,
0x6f6c672e646c090aull,0x203233662e6c6162ull,0x255b202c34662509ull,0x203b5d302b366472ull,
0x333a6469202f2f09ull,0x662e6c756d090a37ull,0x2c35662509203233ull,0x6625202c34662520ull,
0x2020202020203b32ull,0x6d090a202f2f0920ull,0x09203233662e6c75ull,0x336625202c366625ull,
0x20203b356625202cull,0x2f2f092020202020ull,0x6c672e7473090a20ull,0x3233662e6c61626full,
0x2b346472255b0920ull,0x3b366625202c5d30ull,0x3a6469202f2f0920ull,0x2e646461090a3833ull,
...
0x2020202020202020ull,0x240a202f2f092020ull,0x0a3a355f325f744cull,0x353109636f6c2e09ull,
0x090a300930373109ull,0x2020203b74697865ull,0x2020202020202020ull,0x2020202020202020ull,
0x2f09202020202020ull,0x6557444c240a202full,0x6d34315a5f5f646eull,0x4b6574616c75646full,
0x5366506c656e7265ull,0x2f207d090a3a695full,0x6f6d34315a5f202full,0x654b6574616c7564ull,
0x5f5366506c656e72ull,0x00000000000a0a69ull
};
...
static __cudaFatCudaBinary __fatDeviceText __attribute__ ((section (".nvFatBinSegment")))= {0x1ee55a01,0x00000003,0x8ecc680c,(char*)"2b8c20f0bed790a7",(char*)"fastWalshTransform.cu",(char*)" ",__ptxEntries,__cubinEntries,__debugEntries,0,0,0,0,0};
# 5 "/tmp/tmpxft_00001050_00000000-1_fastWalshTransform.cudafe1.stub.c" 2
...
# 6 "/tmp/tmpxft_00001050_00000000-1_fastWalshTransform.cudafe1.stub.c" 2
struct __T20;
struct __T21;
struct __T22;
...
# 21 "/tmp/tmpxft_00001050_00000000-1_fastWalshTransform.cudafe1.stub.c"
static void __sti____cudaRegisterAll_53_tmpxft_00001050_00000000_7_fastWalshTransform_cpp1_ii_c14de7b3(void) __attribute__((__constructor__));
void __device_stub__Z15fwtBatch1KernelPfS_i(float *__par0, float *__par1, int __par2){auto struct __T20 *__T23;
char __[256]; *(char**)&__T23 = __;if (cudaSetupArgument((void*)(char*)&__par0, sizeof(__par0), (size_t)&__T23->__par0 - (size_t)__T23) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par1, sizeof(__par1), (size_t)&__T23->__par1 - (size_t)__T23) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par2, sizeof(__par2), (size_t)&__T23->__par2 - (size_t)__T23) != cudaSuccess) return;{ volatile static char *__f; __f = ((char *)__device_stub__Z15fwtBatch1KernelPfS_i); (void)cudaLaunch(((char *)__device_stub__Z15fwtBatch1KernelPfS_i)); };}
void __device_stub__Z15fwtBatch2KernelPfS_i(float *__par0, float *__par1, int __par2){auto struct __T21 *__T24;
char __[256]; *(char**)&__T24 = __;if (cudaSetupArgument((void*)(char*)&__par0, sizeof(__par0), (size_t)&__T24->__par0 - (size_t)__T24) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par1, sizeof(__par1), (size_t)&__T24->__par1 - (size_t)__T24) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par2, sizeof(__par2), (size_t)&__T24->__par2 - (size_t)__T24) != cudaSuccess) return;{ volatile static char *__f; __f = ((char *)__device_stub__Z15fwtBatch2KernelPfS_i); (void)cudaLaunch(((char *)__device_stub__Z15fwtBatch2KernelPfS_i)); };}
void __device_stub__Z14modulateKernelPfS_i(float *__par0, float *__par1, int __par2){auto struct __T22 *__T218;
char __[256]; *(char**)&__T218 = __;if (cudaSetupArgument((void*)(char*)&__par0, sizeof(__par0), (size_t)&__T218->__par0 - (size_t)__T218) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par1, sizeof(__par1), (size_t)&__T218->__par1 - (size_t)__T218) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par2, sizeof(__par2), (size_t)&__T218->__par2 - (size_t)__T218) != cudaSuccess) return;{ volatile static char *__f; __f = ((char *)__device_stub__Z14modulateKernelPfS_i); (void)cudaLaunch(((char *)__device_stub__Z14modulateKernelPfS_i)); };}
# 37 "/tmp/tmpxft_00001050_00000000-1_fastWalshTransform.cudafe1.stub.c"
static void __sti____cudaRegisterAll_53_tmpxft_00001050_00000000_7_fastWalshTransform_cpp1_ii_c14de7b3(void){__cudaFatCubinHandle = __cudaRegisterFatBinary((void*)(&__fatDeviceText)); atexit(__cudaUnregisterBinaryUtil);__cudaRegisterFunction(__cudaFatCubinHandle, (const char*)__device_stub__Z14modulateKernelPfS_i, (char*)"_Z14modulateKernelPfS_i", "_Z14modulateKernelPfS_i", (-1), (uint3*)0, (uint3*)0, (dim3*)0, (dim3*)0, (int*)0);__cudaRegisterFunction(__cudaFatCubinHandle, (const char*)__device_stub__Z15fwtBatch2KernelPfS_i, (char*)"_Z15fwtBatch2KernelPfS_i", "_Z15fwtBatch2KernelPfS_i", (-1), (uint3*)0, (uint3*)0, (dim3*)0, (dim3*)0, (int*)0);__cudaRegisterFunction(__cudaFatCubinHandle, (const char*)__device_stub__Z15fwtBatch1KernelPfS_i, (char*)"_Z15fwtBatch1KernelPfS_i", "_Z15fwtBatch1KernelPfS_i", (-1), (uint3*)0, (uint3*)0, (dim3*)0, (dim3*)0, (int*)0);__cudaRegisterShared(__cudaFatCubinHandle, (void**)"s_data");}

}
# 197 "fastWalshTransform.cu" 2
