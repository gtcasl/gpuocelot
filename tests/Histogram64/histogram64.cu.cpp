# 1 "/tmp/tmpxft_000011a0_00000000-1_histogram64.cudafe1.cpp"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/tmp/tmpxft_000011a0_00000000-1_histogram64.cudafe1.cpp"
# 1 "histogram64.cu"
...
# 1 "/tmp/tmpxft_000011a0_00000000-1_histogram64.cudafe1.stub.h" 1
...
extern void __device_stub__Z22mergeHistogram64KernelPji(unsigned *, int);


}
# 60 "histogram64.cu" 2
# 174 "histogram64_kernel.cuh"
unsigned *d_Result64;
# 175 "histogram64_kernel.cuh"
unsigned *h_Result64;
# 179 "histogram64_kernel.cuh"
static const int MAX_BLOCK_N = 16384;
# 182 "histogram64_kernel.cuh"
static const int BLOCK_N2 = 32;
# 184 "histogram64_kernel.cuh"
void initHistogram64GPU() {
# 188 "histogram64_kernel.cuh"
h_Result64 = (unsigned *)malloc(((64) * sizeof(unsigned)) * (BLOCK_N2));
# 189 "histogram64_kernel.cuh"
__cudaSafeCall(cudaMalloc((void **)(&d_Result64), (MAX_BLOCK_N) * ((64) * sizeof(unsigned))), "histogram64_kernel.cuh", 189);
# 191 "histogram64_kernel.cuh"
}
# 194 "histogram64_kernel.cuh"
void closeHistogram64GPU() {
# 198 "histogram64_kernel.cuh"
__cudaSafeCall(cudaFree(d_Result64), "histogram64_kernel.cuh", 198);
# 199 "histogram64_kernel.cuh"
free(h_Result64);
# 201 "histogram64_kernel.cuh"
}
# 204 "histogram64_kernel.cuh"
int iDivUp(int a, int b) {
# 205 "histogram64_kernel.cuh"
return ((a % b) != 0) ? (a / b + 1) : (a / b);
# 206 "histogram64_kernel.cuh"
}
# 208 "histogram64_kernel.cuh"
void histogram64GPU(unsigned *
# 209 "histogram64_kernel.cuh"
h_Result, unsigned *
# 210 "histogram64_kernel.cuh"
d_Data, int
# 211 "histogram64_kernel.cuh"
dataN)
# 212 "histogram64_kernel.cuh"
{
# 213 "histogram64_kernel.cuh"
auto const int blockN = iDivUp(dataN / 4, 192 * 63);
# 214 "histogram64_kernel.cuh"
if (blockN > MAX_BLOCK_N) {
# 215 "histogram64_kernel.cuh"
printf("histogram64GPU(): data size exceeds maximum\n");
# 216 "histogram64_kernel.cuh"
return;
# 217 "histogram64_kernel.cuh"
}
# 230 "histogram64_kernel.cuh"
cudaConfigureCall(blockN, 192) ? ((void)0) : __device_stub__Z17histogram64KernelPjS_i(d_Result64, (unsigned *)d_Data, dataN / 4);
# 235 "histogram64_kernel.cuh"
__cutilCheckMsg("histogram64Kernel() execution failed\n", "histogram64_kernel.cuh", 235);
# 237 "histogram64_kernel.cuh"
auto const int FINAL_BLOCK_N = min(BLOCK_N2, blockN);
# 238 "histogram64_kernel.cuh"
if (BLOCK_N2 < blockN) {
# 239 "histogram64_kernel.cuh"
cudaConfigureCall(BLOCK_N2, 64) ? ((void)0) : __device_stub__Z22mergeHistogram64KernelPji(d_Result64, blockN);
# 243 "histogram64_kernel.cuh"
__cutilCheckMsg("mergeHistogram64Kernel() execution failed\n", "histogram64_kernel.cuh", 243);
# 244 "histogram64_kernel.cuh"
}
# 246 "histogram64_kernel.cuh"
__cudaSafeCall(cudaMemcpy(h_Result64, d_Result64, FINAL_BLOCK_N * ((64) * sizeof(unsigned)), cudaMemcpyDeviceToHost), "histogram64_kernel.cuh", 246);
# 247 "histogram64_kernel.cuh"
for (int i = 1; i < FINAL_BLOCK_N; i++) {
# 248 "histogram64_kernel.cuh"
for (int j = 0; j < 64; j++) {
# 249 "histogram64_kernel.cuh"
(h_Result64[j]) += h_Result64[i * 64 + j]; }
# 250 "histogram64_kernel.cuh"
}
# 251 "histogram64_kernel.cuh"
for (int i = 0; i < 64; i++) {
# 252 "histogram64_kernel.cuh"
(h_Result[i]) = h_Result64[i]; }
# 255 "histogram64_kernel.cuh"
}
# 73 "histogram64.cu"
static const int DATA_N = 10000000;
# 74 "histogram64.cu"
...
# 85 "histogram64.cu"
auto unsigned *h_ResultCPU; auto unsigned *h_ResultGPU;
# 88 "histogram64.cu"
...
# 102 "histogram64.cu"
__cutilCheckError(cutCreateTimer(&hTimer), "histogram64.cu", 102);
# 104 "histogram64.cu"
...
# 107 "histogram64.cu"
h_ResultCPU = (unsigned *)malloc((64) * sizeof(unsigned));
# 108 "histogram64.cu"
h_ResultGPU = (unsigned *)malloc((64) * sizeof(unsigned));
# 110 "histogram64.cu"
...
# 116 "histogram64.cu"
__cudaSafeCall(cudaMalloc((void **)(&d_Data), DATA_SIZE), "histogram64.cu", 116);
# 117 "histogram64.cu"
__cudaSafeCall(cudaMemcpy(d_Data, h_Data, DATA_SIZE, cudaMemcpyHostToDevice), "histogram64.cu", 117);
# 120 "histogram64.cu"
printf("Running GPU histogram (%i iterations)...\n", NUM_ITERATIONS);
# 121 "histogram64.cu"
initHistogram64GPU();
# 122 "histogram64.cu"
__cudaSafeCall(cudaThreadSynchronize(), "histogram64.cu", 122);
# 123 "histogram64.cu"
__cutilCheckError(cutResetTimer(hTimer), "histogram64.cu", 123);
# 124 "histogram64.cu"
__cutilCheckError(cutStartTimer(hTimer), "histogram64.cu", 124);
# 126 "histogram64.cu"
for (iter = 0; iter < NUM_ITERATIONS; iter++) {
# 127 "histogram64.cu"
histogram64GPU(h_ResultGPU, (unsigned *)d_Data, DATA_N); }
# 128 "histogram64.cu"
__cudaSafeCall(cudaThreadSynchronize(), "histogram64.cu", 128);
# 129 "histogram64.cu"
__cutilCheckError(cutStopTimer(hTimer), "histogram64.cu", 129);
# 130 "histogram64.cu"
...
printf("...histogramCPU()\n");
# 136 "histogram64.cu"
__cutilCheckError(cutResetTimer(hTimer), "histogram64.cu", 136);
# 137 "histogram64.cu"
__cutilCheckError(cutStartTimer(hTimer), "histogram64.cu", 137);
# 138 "histogram64.cu"
histogram64CPU(h_ResultCPU, (unsigned *)h_Data, DATA_N / 4);
# 143 "histogram64.cu"
__cutilCheckError(cutStopTimer(hTimer), "histogram64.cu", 143);
# 144 "histogram64.cu"
...
# 159 "histogram64.cu"
__cutilCheckError(cutDeleteTimer(hTimer), "histogram64.cu", 159);
# 160 "histogram64.cu"
__cudaSafeCall(cudaFree(d_Data), "histogram64.cu", 160);
# 161 "histogram64.cu"
free(h_ResultGPU);
0x20203b7469786509ull,0x2020202020202020ull,0x2020202020202020ull,0x0920202020202020ull,
0x57444c240a202f2full,0x32325a5f5f646e65ull,0x736948656772656dull,0x34366d6172676f74ull,
0x6a506c656e72654bull,0x2f2f207d090a3a69ull,0x72656d32325a5f20ull,0x676f747369486567ull,
0x72654b34366d6172ull,0x0a0a696a506c656eull,0x0000000000000000ull
};
...
static __cudaFatCudaBinary __fatDeviceText __attribute__ ((section (".nvFatBinSegment")))= {0x1ee55a01,0x00000003,0x8ecc680c,(char*)"20f1ba424e770091",(char*)"histogram64.cu",(char*)" ",__ptxEntries,__cubinEntries,__debugEntries,0,0,0,0,0};
# 5 "/tmp/tmpxft_000011a0_00000000-1_histogram64.cudafe1.stub.c" 2
...
# 6 "/tmp/tmpxft_000011a0_00000000-1_histogram64.cudafe1.stub.c" 2
struct __T20;
struct __T21;
struct __T20 {unsigned *__par0;unsigned *__par1;int __par2;int __dummy_field;};
struct __T21 {unsigned *__par0;int __par1;int __dummy_field;};
...
static void __sti____cudaRegisterAll_46_tmpxft_000011a0_00000000_4_histogram64_cpp1_ii_c14de7b3(void) __attribute__((__constructor__));
void __device_stub__Z17histogram64KernelPjS_i(unsigned *__par0, unsigned *__par1, int __par2){auto struct __T20 *__T22;
char __[256]; *(char**)&__T22 = __;if (cudaSetupArgument((void*)(char*)&__par0, sizeof(__par0), (size_t)&__T22->__par0 - (size_t)__T22) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par1, sizeof(__par1), (size_t)&__T22->__par1 - (size_t)__T22) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par2, sizeof(__par2), (size_t)&__T22->__par2 - (size_t)__T22) != cudaSuccess) return;{ volatile static char *__f; __f = ((char *)__device_stub__Z17histogram64KernelPjS_i); (void)cudaLaunch(((char *)__device_stub__Z17histogram64KernelPjS_i)); };}
void __device_stub__Z22mergeHistogram64KernelPji(unsigned *__par0, int __par1){auto struct __T21 *__T23;
char __[256]; *(char**)&__T23 = __;if (cudaSetupArgument((void*)(char*)&__par0, sizeof(__par0), (size_t)&__T23->__par0 - (size_t)__T23) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par1, sizeof(__par1), (size_t)&__T23->__par1 - (size_t)__T23) != cudaSuccess) return;{ volatile static char *__f; __f = ((char *)__device_stub__Z22mergeHistogram64KernelPji); (void)cudaLaunch(((char *)__device_stub__Z22mergeHistogram64KernelPji)); };}
...
static void __sti____cudaRegisterAll_46_tmpxft_000011a0_00000000_4_histogram64_cpp1_ii_c14de7b3(void){__cudaFatCubinHandle = __cudaRegisterFatBinary((void*)(&__fatDeviceText)); atexit(__cudaUnregisterBinaryUtil);__cudaRegisterFunction(__cudaFatCubinHandle, (const char*)__device_stub__Z22mergeHistogram64KernelPji, (char*)"_Z22mergeHistogram64KernelPji", "_Z22mergeHistogram64KernelPji", (-1), (uint3*)0, (uint3*)0, (dim3*)0, (dim3*)0, (int*)0);__cudaRegisterFunction(__cudaFatCubinHandle, (const char*)__device_stub__Z17histogram64KernelPjS_i, (char*)"_Z17histogram64KernelPjS_i", "_Z17histogram64KernelPjS_i", (-1), (uint3*)0, (uint3*)0, (dim3*)0, (dim3*)0, (int*)0);}

}
# 170 "histogram64.cu" 2
