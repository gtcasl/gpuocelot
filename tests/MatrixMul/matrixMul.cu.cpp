# 1 "/tmp/tmpxft_000013c3_00000000-1_matrixMul.cudafe1.cpp"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/tmp/tmpxft_000013c3_00000000-1_matrixMul.cudafe1.cpp"
# 1 "matrixMul.cu"
...
# 1 "/tmp/tmpxft_000013c3_00000000-1_matrixMul.cudafe1.stub.h" 1
...
# 64 "matrixMul.cu"
extern void printDiff(float *, float *, int, int);
# 67 "matrixMul.cu"
...
srand(2006);
# 95 "matrixMul.cu"
auto unsigned size_A = ((3 * 16) * (5 * 16));
# 96 "matrixMul.cu"
auto unsigned mem_size_A = (sizeof(float) * size_A);
# 97 "matrixMul.cu"
auto float *h_A = ((float *)malloc(mem_size_A));
# 98 "matrixMul.cu"
auto unsigned size_B = ((8 * 16) * (3 * 16));
# 99 "matrixMul.cu"
auto unsigned mem_size_B = (sizeof(float) * size_B);
# 100 "matrixMul.cu"
auto float *h_B = ((float *)malloc(mem_size_B));
# 103 "matrixMul.cu"
...
auto float *d_A;
# 108 "matrixMul.cu"
__cudaSafeCall(cudaMalloc((void **)(&d_A), mem_size_A), "matrixMul.cu", 108);
# 109 "matrixMul.cu"
auto float *d_B;
# 110 "matrixMul.cu"
__cudaSafeCall(cudaMalloc((void **)(&d_B), mem_size_B), "matrixMul.cu", 110);
# 113 "matrixMul.cu"
__cudaSafeCall(cudaMemcpy(d_A, h_A, mem_size_A, cudaMemcpyHostToDevice), "matrixMul.cu", 114);
# 115 "matrixMul.cu"
__cudaSafeCall(cudaMemcpy(d_B, h_B, mem_size_B, cudaMemcpyHostToDevice), "matrixMul.cu", 116);
# 119 "matrixMul.cu"
auto unsigned size_C = ((8 * 16) * (5 * 16));
# 120 "matrixMul.cu"
auto unsigned mem_size_C = (sizeof(float) * size_C);
# 121 "matrixMul.cu"
auto float *d_C;
# 122 "matrixMul.cu"
__cudaSafeCall(cudaMalloc((void **)(&d_C), mem_size_C), "matrixMul.cu", 122);
# 125 "matrixMul.cu"
auto float *h_C = ((float *)malloc(mem_size_C));
# 128 "matrixMul.cu"
auto unsigned timer = (0);
# 129 "matrixMul.cu"
__cutilCheckError(cutCreateTimer(&timer), "matrixMul.cu", 129);
# 130 "matrixMul.cu"
__cutilCheckError(cutStartTimer(timer), "matrixMul.cu", 130);
# 133 "matrixMul.cu"
auto dim3 threads(16, 16);
# 134 "matrixMul.cu"
auto dim3 grid((8 * 16) / threads.x, (5 * 16) / threads.y);
# 137 "matrixMul.cu"
cudaConfigureCall(grid, threads) ? ((void)0) : __device_stub__Z9matrixMulPfS_S_ii(d_C, d_A, d_B, 3 * 16, 8 * 16);
# 140 "matrixMul.cu"
__cutilCheckMsg("Kernel execution failed", "matrixMul.cu", 140);
# 143 "matrixMul.cu"
__cudaSafeCall(cudaMemcpy(h_C, d_C, mem_size_C, cudaMemcpyDeviceToHost), "matrixMul.cu", 144);
# 147 "matrixMul.cu"
__cutilCheckError(cutStopTimer(timer), "matrixMul.cu", 147);
# 148 "matrixMul.cu"
printf("Processing time: %f (ms) \n", cutGetTimerValue(timer));
# 149 "matrixMul.cu"
__cutilCheckError(cutDeleteTimer(timer), "matrixMul.cu", 149);
# 152 "matrixMul.cu"
auto float *reference = ((float *)malloc(mem_size_C));
# 153 "matrixMul.cu"
computeGold(reference, h_A, h_B, 5 * 16, 3 * 16, 8 * 16);
# 156 "matrixMul.cu"
auto CUTBoolean res = cutCompareL2fe(reference, h_C, size_C, (9.999999975e-07F));
# 157 "matrixMul.cu"
printf("Test %s \n", (1 == res) ? ((const char *)"PASSED") : ((const char *)"FAILED"));
# 158 "matrixMul.cu"
if (res != 1) { printDiff(reference, h_C, 8 * 16, 5 * 16); }
# 161 "matrixMul.cu"
free(h_A);
# 162 "matrixMul.cu"
free(h_B);
...
free(reference);
# 165 "matrixMul.cu"
__cudaSafeCall(cudaFree(d_A), "matrixMul.cu", 165);
# 166 "matrixMul.cu"
__cudaSafeCall(cudaFree(d_B), "matrixMul.cu", 166);
# 167 "matrixMul.cu"
__cudaSafeCall(cudaFree(d_C), "matrixMul.cu", 167);
# 169 "matrixMul.cu"
cudaThreadExit();
# 170 "matrixMul.cu"
}
...
# 1 "/tmp/tmpxft_000013c3_00000000-1_matrixMul.cudafe1.stub.c" 1

extern "C" {

# 1 "/tmp/tmpxft_000013c3_00000000-3_matrixMul.fatbin.c" 1
...
# 2 "/tmp/tmpxft_000013c3_00000000-3_matrixMul.fatbin.c" 2
...
0x20656d616e090a7bull,0x74616d395a5f203dull,0x66506c754d786972ull,0x090a69695f535f53ull,
0x30203d206d656d6cull,0x3d206d656d73090aull,0x72090a3639303220ull,0x3431203d20206765ull,
...
0x3065343030306178ull,0x3032343078302064ull,0x3178302030383730ull,0x2035303431303030ull,
0x3763303034347830ull,0x783009090a203038ull,0x3930633033303034ull,0x3030303030783020ull,
0x3036783020303837ull,0x3020393065303230ull,0x3837383030303078ull,0x33783009090a2030ull,
0x2039303430303130ull,0x3730303134637830ull,0x3030367830203038ull,0x7830203530633032ull,
0x3038373830303030ull,0x3033783009090a20ull,0x3020313232303430ull,0x3837303031346378ull,
0x3830313278302030ull,0x3178302038303466ull,0x2034303038303030ull,0x333032783009090aull,
0x7830203532343866ull,0x6666666666666630ull,0x3430303061783020ull,0x3430783020313163ull,
0x0a20303837303032ull,0x3030303178300909ull,0x3078302031303866ull,0x2030383763333034ull,
0x3630303830647830ull,0x3430307830206430ull,0x090a203038373030ull,0x3138303033783009ull,
0x6336783020646633ull,0x3020386337343030ull,0x3238303430303378ull,0x3031346378302039ull,
0x09090a2030383730ull,0x3430303030617830ull,0x3034307830203531ull,0x7830203038373030ull,
0x3931363030303061ull,0x3030303430783020ull,0x3009090a20303837ull,0x3030633530303178ull,
0x3030303078302033ull,0x3178302030383230ull,0x2038303666303031ull,0x3466303031317830ull,
0x783009090a203430ull,0x6332383135303034ull,0x3133303034783020ull,0x3036783020633138ull,
0x25202c3932722509ull,0x38327225202c3172ull,0x2f0920202020203bull,0x2e6c756d090a202full,
0x09203233732e6f6cull,0x7225202c30337225ull,0x203b3631202c3932ull,0x090a202f2f092020ull,
0x203233732e646461ull,0x25202c3133722509ull,0x337225202c383172ull,0x2f09202020203b30ull,
0x2e646461090a202full,0x3372250920323373ull,0x202c397225202c32ull,0x2020203b31337225ull,
0x090a202f2f092020ull,0x2e3436732e747663ull,0x6472250920323373ull,0x32337225202c3532ull,
...
0x326472255b092032ull,0x6625202c5d302b37ull,0x6469202f2f093b31ull,0x6c2e090a3232313aull,
0x323109353109636full,0x697865090a300938ull,0x2020202020203b74ull,0x2020202020202020ull,
0x2020202020202020ull,0x0a202f2f09202020ull,0x5f646e6557444c24ull,0x697274616d395a5full,
0x5f5366506c754d78ull,0x7d090a3a69695f53ull,0x6d395a5f202f2f20ull,0x6c754d7869727461ull,
0x69695f535f536650ull,0x0000000000000a0aull
};
...
static __cudaFatCudaBinary __fatDeviceText __attribute__ ((section (".nvFatBinSegment")))= {0x1ee55a01,0x00000003,0x8ecc680c,(char*)"9706f1e8332f1976",(char*)"matrixMul.cu",(char*)" ",__ptxEntries,__cubinEntries,__debugEntries,0,0,0,0,0};
# 5 "/tmp/tmpxft_000013c3_00000000-1_matrixMul.cudafe1.stub.c" 2
...
# 6 "/tmp/tmpxft_000013c3_00000000-1_matrixMul.cudafe1.stub.c" 2
struct __T20;
struct __T20 {float *__par0;float *__par1;float *__par2;int __par3;int __par4;int __dummy_field;volatile char __dummy[4];};



static void __sti____cudaRegisterAll_44_tmpxft_000013c3_00000000_7_matrixMul_cpp1_ii_c14de7b3(void) __attribute__((__constructor__));
void __device_stub__Z9matrixMulPfS_S_ii(float *__par0, float *__par1, float *__par2, int __par3, int __par4){auto struct __T20 *__T21;
char __[256]; *(char**)&__T21 = __;if (cudaSetupArgument((void*)(char*)&__par0, sizeof(__par0), (size_t)&__T21->__par0 - (size_t)__T21) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par1, sizeof(__par1), (size_t)&__T21->__par1 - (size_t)__T21) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par2, sizeof(__par2), (size_t)&__T21->__par2 - (size_t)__T21) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par3, sizeof(__par3), (size_t)&__T21->__par3 - (size_t)__T21) != cudaSuccess) return;if (cudaSetupArgument((void*)(char*)&__par4, sizeof(__par4), (size_t)&__T21->__par4 - (size_t)__T21) != cudaSuccess) return;{ volatile static char *__f; __f = ((char *)__device_stub__Z9matrixMulPfS_S_ii); (void)cudaLaunch(((char *)__device_stub__Z9matrixMulPfS_S_ii)); };}



static void __sti____cudaRegisterAll_44_tmpxft_000013c3_00000000_7_matrixMul_cpp1_ii_c14de7b3(void){__cudaFatCubinHandle = __cudaRegisterFatBinary((void*)(&__fatDeviceText)); atexit(__cudaUnregisterBinaryUtil);__cudaRegisterFunction(__cudaFatCubinHandle, (const char*)__device_stub__Z9matrixMulPfS_S_ii, (char*)"_Z9matrixMulPfS_S_ii", "_Z9matrixMulPfS_S_ii", (-1), (uint3*)0, (uint3*)0, (dim3*)0, (dim3*)0, (int*)0);}
