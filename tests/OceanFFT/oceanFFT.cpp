...
// GL functionality
CUTBoolean initGL(bool bFBODisplay);
...
{
    printf("CUDA FFT Ocean Simulation\n\n"
...
void runTest(int argc, char** argv)
{
    ...
    // initialize GL
    if(CUTFalse == initGL(bFBODisplay)) {
        cudaThreadExit();
        return;
    }
    ...
    if (bQAReadback) {
        if (bFBODisplay) {
            g_CheckRender = new CheckFBO(windowW, windowH, 4);
        } else {
...
void runCuda()
{
    ...
    float *hptr;
    cutilSafeCall(cudaGLMapBufferObject((void**)&hptr, heightVertexBuffer));
    cufftSafeCall( cufftExecC2R(fftPlan, (cufftComplex *) d_ht, hptr) );

    // calculate slope for shading
    float2 *sptr;
    cutilSafeCall(cudaGLMapBufferObject((void**)&sptr, slopeVertexBuffer));
    cudaCalculateSlopeKernel(hptr, sptr, meshW, meshH);
...
void display()
{
    ...
    if (g_CheckRender && g_CheckRender->IsQAReadback() && g_Verify) {
        ...
        g_CheckRender->readback( windowW, windowH, NULL );
        g_CheckRender->savePPM ( sOriginal[0], true, NULL);
        if (!g_CheckRender->PPMvsPPM( sOriginal[0], sReference[0], MAX_EPSILON )) {
            g_TotalErrors++;   
        }
        g_Verify = false;
    }
