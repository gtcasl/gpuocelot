# 1 "/tmp/tmpxft_00000eb0_00000000-22_main.cudafe1.cpp"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/tmp/tmpxft_00000eb0_00000000-22_main.cudafe1.cpp"
# 1 "main.cu"
...
{
# 109 "main.cu"
auto unsigned input_data_size = mat_size;
# 110 "main.cu"
auto char *diag_path = cutFindFilePath("diagonal.dat", exec_path);
# 111 "main.cu"
__cutilCondition((0) != diag_path, (char *)("main.cu"), 111);
# 112 "main.cu"
__cutilCheckError(cutReadFilef(diag_path, &(input.a), &input_data_size), "main.cu", 112);
# 114 "main.cu"
auto char *sdiag_path = cutFindFilePath("superdiagonal.dat", exec_path);
# 115 "main.cu"
__cutilCondition((0) != sdiag_path, (char *)("main.cu"), 115);
# 116 "main.cu"
__cutilCheckError(cutReadFilef(sdiag_path, &(input.b), &input_data_size, 1), "main.cu", 117);
# 119 "main.cu"
cutFree(diag_path);
# 120 "main.cu"
cutFree(sdiag_path);
...
# 124 "main.cu"
__cudaSafeCall(cudaMalloc((void **)(&(input.g_a)), sizeof(float) * mat_size), "main.cu", 125);
# 126 "main.cu"
__cudaSafeCall(cudaMalloc((void **)(&(input.g_b_raw)), sizeof(float) * mat_size), "main.cu", 127);
# 130 "main.cu"
__cudaSafeCall(cudaMemcpy(input.g_a, input.a, sizeof(float) * mat_size, cudaMemcpyHostToDevice), "main.cu", 131);
# 132 "main.cu"
__cudaSafeCall(cudaMemcpy(input.g_b_raw, input.b, sizeof(float) * mat_size, cudaMemcpyHostToDevice), "main.cu", 134);
# 135 "main.cu"
...
freePtr(input.b);
# 148 "main.cu"
__cudaSafeCall(cudaFree(input.g_a), "main.cu", 148);
# 149 "main.cu"
(input.g_a) = (__null);
# 150 "main.cu"
__cudaSafeCall(cudaFree(input.g_b_raw), "main.cu", 150);
# 151 "main.cu"
...
# 172 "main.cu"
__cutilCondition((mat_size) < (1 << 16), (char *)("main.cu"), 172);
# 174 "main.cu"
user_defined = (1);
# 175 "main.cu"
}
...
# 257 "main.cu"
__cutilCheckError(cutCreateTimer(&timer), "main.cu", 257);
# 258 "main.cu"
__cutilCheckError(cutCreateTimer(&timer_total), "main.cu", 258);
# 261 "main.cu"
auto unsigned mat_size = (2048);
# 263 "main.cu"
auto unsigned user_defined = (0);
# 265 "main.cu"
auto float precision = (9.999999747e-06F);
# 269 "main.cu"
auto unsigned iters_timing = (1);
# 271 "main.cu"
auto char *result_file = ((char *)("eigenvalues.dat"));
# 274 "main.cu"
...
# 290 "main.cu"
auto float lg = (3.402823466e+38F);
# 291 "main.cu"
auto float ug = (-(3.402823466e+38F));
# 292 "main.cu"
...
# 1 "/tmp/tmpxft_00000eb0_00000000-22_main.cudafe1.stub.c" 1

extern "C" {

# 1 "/tmp/tmpxft_00000eb0_00000000-24_main.fatbin.c" 1
...
# 2 "/tmp/tmpxft_00000eb0_00000000-24_main.fatbin.c" 2
...
0x5f37372d30303030ull,0x7070632e6e69616dull,0x6d742f2820692e33ull,0x2e23494263632f70ull,
0x0a294b4c45753736ull,0x2d2d2d2d2d2f2f09ull,0x2d2d2d2d2d2d2d2dull,0x2d2d2d2d2d2d2d2dull,
0x2d2d2d2d2d2d2d2dull,0x2d2d2d2d2d2d2d2dull,0x2d2d2d2d2d2d2d2dull,0x2d2d2d2d2d2d2d2dull,
...
0x706d742f22093209ull,0x5f746678706d742full,0x3062653030303030ull,0x303030303030305full,
0x69616d5f36372d30ull,0x6566616475632e6eull,0x090a227570672e32ull,0x093309656c69662eull,
...
0x2e656d69742f6564ull,0x6c69662e090a2268ull,0x682f220935310965ull,0x616964672f656d6full,
0x706d65742f736f6dull,0x69622f616475632full,0x6e692f2e2e2f2f6eull,0x6f632f6564756c63ull,
0x6e75665f6e6f6d6dull,0x682e736e6f697463ull,0x656c69662e090a22ull,0x6f682f2209363109ull,
0x6d616964672f656dull,0x2f706d65742f736full,0x6e69622f61647563ull,0x636e692f2e2e2f2full,
0x7472632f6564756cull,0x616d5f636e75662full,0x090a22682e6f7263ull,0x373109656c69662eull,
...
0x0a22682e73746e61ull,0x3209656c69662e09ull,0x656d6f682f220930ull,0x736f6d616964672full,
0x75632f706d65742full,0x2f2f6e69622f6164ull,0x756c636e692f2e2eull,0x31315f6d732f6564ull,
0x5f63696d6f74615full,0x6e6f6974636e7566ull,0x662e090a22682e73ull,0x2209313209656c69ull,
...
0x2e337874705f6c62ull,0x0000000a0a0a2268ull
};
...
static __cudaFatCudaBinary __fatDeviceText __attribute__ ((section (".nvFatBinSegment")))= {0x1ee55a01,0x00000003,0x8ecc680c,(char*)"d5254446ac2d59fb",(char*)"main.cu",(char*)" ",__ptxEntries,__cubinEntries,__debugEntries,0,0,0,0,0};
# 5 "/tmp/tmpxft_00000eb0_00000000-22_main.cudafe1.stub.c" 2
...
# 6 "/tmp/tmpxft_00000eb0_00000000-22_main.cudafe1.stub.c" 2
static void __sti____cudaRegisterAll_40_tmpxft_00000eb0_00000000_74_main_cpp1_ii_c14de7b3(void) __attribute__((__constructor__));
static void __sti____cudaRegisterAll_40_tmpxft_00000eb0_00000000_74_main_cpp1_ii_c14de7b3(void){__cudaFatCubinHandle = __cudaRegisterFatBinary((void*)(&__fatDeviceText)); atexit(__cudaUnregisterBinaryUtil);}
