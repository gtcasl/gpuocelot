                     ...
                     int bUsePBO)
{
    ...
    {
				/*
				printf("integrateBodies<false><<< (%d, %d) x (%d, %d), %d bytes >>>() called\n",
					grid.x, grid.y, threads.x, threads.y, sharedMemSize);
				printf("  newPos = 0x%x, oldPos = 0x%x, newVel = 0x%x, oldVel = 0x%x\n",
					(unsigned long)newPos, (unsigned long)oldPos, (unsigned long)newVel, (unsigned long)oldVel);
				fflush(stdout);
				*/
    ...
    {
				/*
				printf("integrateBodies<true><<< (%d, %d) x (%d, %d), %d bytes >>>() called\n",
					grid.x, grid.y, threads.x, threads.y, sharedMemSize);
				printf("  newPos = 0x%x, oldPos = 0x%x, newVel = 0x%x, oldVel = 0x%x\n",
					(unsigned long)newPos, (unsigned long)oldPos, (unsigned long)newVel, (unsigned long)oldVel);
				fflush(stdout); */
