    ...
    else
    {
        ...
        float* cpuPos  = nbodyCPU->getArray(BodySystem::BODYSYSTEM_POSITION);
				...
				for (int i = 0; i < numBodies * 3; i++) {
					if (fabs(cudaPos[i] - cpuPos[i]) > EPSILON) {
						printf("error on body [%d].%s: cpu: %f, cuda: %f\n",
							(i/3), dimensions[i%3], cpuPos[i], cudaPos[i]);
						if (++errors > 10) { break; }
					}
				}
...
void display()
{
    ...
    fpsCount++;
    // this displays the frame rate updated every second 
...
main( int argc, char** argv) 
{
	...
	numBodies = 1024;
		...
		compareToCPU = (compareToCPU && !visual);
		benchmark = (benchmark && !visual);
