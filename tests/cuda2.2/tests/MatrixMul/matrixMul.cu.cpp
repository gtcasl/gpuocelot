...
# 62 "tests/MatrixMul/matrixMul.cu"
void runTest(int , char ** );
# 63 "tests/MatrixMul/matrixMul.cu"
void randomInit(float *, int);
# 64 "tests/MatrixMul/matrixMul.cu"
void printDiff(float *, float *, int, int);
# 67 "tests/MatrixMul/matrixMul.cu"
extern "C" void computeGold(float *, const float *, const float *, unsigned, unsigned, unsigned);
# 73 "tests/MatrixMul/matrixMul.cu"
int main(int argc, char **argv)
# 74 "tests/MatrixMul/matrixMul.cu"
{
# 75 "tests/MatrixMul/matrixMul.cu"
runTest(argc, argv);
# 77 "tests/MatrixMul/matrixMul.cu"
__cutilExit(argc, argv); return 0;
# 78 "tests/MatrixMul/matrixMul.cu"
}
# 84 "tests/MatrixMul/matrixMul.cu"
void runTest(int argc, char **argv)
# 85 "tests/MatrixMul/matrixMul.cu"
{
# 86 "tests/MatrixMul/matrixMul.cu"
if (cutCheckCmdLineFlag(argc, (const char **)argv, "device")) {
# 87 "tests/MatrixMul/matrixMul.cu"
cutilDeviceInit(argc, argv); } else {
# 89 "tests/MatrixMul/matrixMul.cu"
cudaSetDevice(cutGetMaxGflopsDeviceId()); }
# 92 "tests/MatrixMul/matrixMul.cu"
srand(2006);
# 95 "tests/MatrixMul/matrixMul.cu"
unsigned size_A = ((3 * 16) * (5 * 16));
# 96 "tests/MatrixMul/matrixMul.cu"
unsigned mem_size_A = (sizeof(float) * size_A);
# 97 "tests/MatrixMul/matrixMul.cu"
float *h_A = ((float *)malloc(mem_size_A));
# 98 "tests/MatrixMul/matrixMul.cu"
unsigned size_B = ((8 * 16) * (3 * 16));
# 99 "tests/MatrixMul/matrixMul.cu"
unsigned mem_size_B = (sizeof(float) * size_B);
# 100 "tests/MatrixMul/matrixMul.cu"
float *h_B = ((float *)malloc(mem_size_B));
# 103 "tests/MatrixMul/matrixMul.cu"
randomInit(h_A, size_A);
# 104 "tests/MatrixMul/matrixMul.cu"
randomInit(h_B, size_B);
# 107 "tests/MatrixMul/matrixMul.cu"
float *d_A;
# 108 "tests/MatrixMul/matrixMul.cu"
__cudaSafeCall(cudaMalloc((void **)(&d_A), mem_size_A), "tests/MatrixMul/matrixMul.cu", 108);
# 109 "tests/MatrixMul/matrixMul.cu"
float *d_B;
# 110 "tests/MatrixMul/matrixMul.cu"
__cudaSafeCall(cudaMalloc((void **)(&d_B), mem_size_B), "tests/MatrixMul/matrixMul.cu", 110);
# 113 "tests/MatrixMul/matrixMul.cu"
__cudaSafeCall(cudaMemcpy(d_A, h_A, mem_size_A, cudaMemcpyHostToDevice), "tests/MatrixMul/matrixMul.cu", 114);
# 115 "tests/MatrixMul/matrixMul.cu"
__cudaSafeCall(cudaMemcpy(d_B, h_B, mem_size_B, cudaMemcpyHostToDevice), "tests/MatrixMul/matrixMul.cu", 116);
# 119 "tests/MatrixMul/matrixMul.cu"
unsigned size_C = ((8 * 16) * (5 * 16));
# 120 "tests/MatrixMul/matrixMul.cu"
unsigned mem_size_C = (sizeof(float) * size_C);
# 121 "tests/MatrixMul/matrixMul.cu"
float *d_C;
# 122 "tests/MatrixMul/matrixMul.cu"
__cudaSafeCall(cudaMalloc((void **)(&d_C), mem_size_C), "tests/MatrixMul/matrixMul.cu", 122);
# 125 "tests/MatrixMul/matrixMul.cu"
float *h_C = ((float *)malloc(mem_size_C));
# 128 "tests/MatrixMul/matrixMul.cu"
unsigned timer = (0);
# 129 "tests/MatrixMul/matrixMul.cu"
__cutilCheckError(cutCreateTimer(&timer), "tests/MatrixMul/matrixMul.cu", 129);
# 130 "tests/MatrixMul/matrixMul.cu"
__cutilCheckError(cutStartTimer(timer), "tests/MatrixMul/matrixMul.cu", 130);
# 133 "tests/MatrixMul/matrixMul.cu"
dim3 threads(16, 16);
# 134 "tests/MatrixMul/matrixMul.cu"
dim3 grid((8 * 16) / (threads.x), (5 * 16) / (threads.y));
# 137 "tests/MatrixMul/matrixMul.cu"
cudaConfigureCall(grid, threads) ? ((void)0) : matrixMul(d_C, d_A, d_B, 3 * 16, 8 * 16);
# 140 "tests/MatrixMul/matrixMul.cu"
__cutilCheckMsg("Kernel execution failed", "tests/MatrixMul/matrixMul.cu", 140);
# 143 "tests/MatrixMul/matrixMul.cu"
__cudaSafeCall(cudaMemcpy(h_C, d_C, mem_size_C, cudaMemcpyDeviceToHost), "tests/MatrixMul/matrixMul.cu", 144);
# 147 "tests/MatrixMul/matrixMul.cu"
__cutilCheckError(cutStopTimer(timer), "tests/MatrixMul/matrixMul.cu", 147);
# 148 "tests/MatrixMul/matrixMul.cu"
printf("Processing time: %f (ms) \n", cutGetTimerValue(timer));
# 149 "tests/MatrixMul/matrixMul.cu"
__cutilCheckError(cutDeleteTimer(timer), "tests/MatrixMul/matrixMul.cu", 149);
# 152 "tests/MatrixMul/matrixMul.cu"
float *reference = ((float *)malloc(mem_size_C));
# 153 "tests/MatrixMul/matrixMul.cu"
computeGold(reference, h_A, h_B, 5 * 16, 3 * 16, 8 * 16);
# 156 "tests/MatrixMul/matrixMul.cu"
CUTBoolean res = cutCompareL2fe(reference, h_C, size_C, (9.999999975e-07F));
# 157 "tests/MatrixMul/matrixMul.cu"
printf("Test %s \n", (1 == res) ? ("PASSED") : ("FAILED"));
# 158 "tests/MatrixMul/matrixMul.cu"
if (res != 1) { printDiff(reference, h_C, 8 * 16, 5 * 16); }
# 161 "tests/MatrixMul/matrixMul.cu"
free(h_A);
# 162 "tests/MatrixMul/matrixMul.cu"
free(h_B);
# 163 "tests/MatrixMul/matrixMul.cu"
free(h_C);
# 164 "tests/MatrixMul/matrixMul.cu"
free(reference);
# 165 "tests/MatrixMul/matrixMul.cu"
__cudaSafeCall(cudaFree(d_A), "tests/MatrixMul/matrixMul.cu", 165);
# 166 "tests/MatrixMul/matrixMul.cu"
__cudaSafeCall(cudaFree(d_B), "tests/MatrixMul/matrixMul.cu", 166);
# 167 "tests/MatrixMul/matrixMul.cu"
__cudaSafeCall(cudaFree(d_C), "tests/MatrixMul/matrixMul.cu", 167);
# 169 "tests/MatrixMul/matrixMul.cu"
cudaThreadExit();
# 170 "tests/MatrixMul/matrixMul.cu"
}
# 173 "tests/MatrixMul/matrixMul.cu"
void randomInit(float *data, int size)
# 174 "tests/MatrixMul/matrixMul.cu"
{
# 175 "tests/MatrixMul/matrixMul.cu"
for (int i = 0; i < size; ++i) {
# 176 "tests/MatrixMul/matrixMul.cu"
(data[i]) = ((rand()) / ((float)2147483647)); }
# 177 "tests/MatrixMul/matrixMul.cu"
}
# 179 "tests/MatrixMul/matrixMul.cu"
void printDiff(float *data1, float *data2, int width, int height)
# 180 "tests/MatrixMul/matrixMul.cu"
{
# 181 "tests/MatrixMul/matrixMul.cu"
int i, j, k;
# 182 "tests/MatrixMul/matrixMul.cu"
int error_count = 0;
# 183 "tests/MatrixMul/matrixMul.cu"
for (j = 0; j < height; j++) {
# 184 "tests/MatrixMul/matrixMul.cu"
for (i = 0; i < width; i++) {
# 185 "tests/MatrixMul/matrixMul.cu"
k = ((j * width) + i);
# 186 "tests/MatrixMul/matrixMul.cu"
if ((data1[k]) != (data2[k])) {
# 187 "tests/MatrixMul/matrixMul.cu"
printf("diff(%d,%d) CPU=%4.4f, GPU=%4.4f n", i, j, data1[k], data2[k]);
# 188 "tests/MatrixMul/matrixMul.cu"
error_count++;
# 189 "tests/MatrixMul/matrixMul.cu"
}
# 190 "tests/MatrixMul/matrixMul.cu"
}
# 191 "tests/MatrixMul/matrixMul.cu"
}
# 192 "tests/MatrixMul/matrixMul.cu"
...
".align 8\n"
"fatbinData:\n"
".quad 0x00100001ba55ed50,0x0000000000001ad8,0x0000005801000002,0x00000000000008d0\n"
".quad 0x0000000000000000,0x0000001400010004,0x0000001c00000038,0x0000000000000015\n"
".quad 0x0000000000000000,0x614d2f7374736574,0x2f6c754d78697274,0x754d78697274616d\n"
".quad 0x0000000075632e6c,0x33010102464c457f,0x0000000000000004,0x0000000100be0002\n"
".quad 0x0000000000000000,0x0000000000000828,0x0000000000000040,0x0038004000140514\n"
".quad 0x0001000900400003,0x0000000000000000,0x0000000000000000,0x0000000000000000\n"
".quad 0x0000000000000000,0x0000000000000000,0x0000000000000000,0x0000000000000000\n"
".quad 0x0000000000000000,0x0000000300000001,0x0000000000000000,0x0000000000000000\n"
".quad 0x0000000000000280,0x00000000000000bb,0x0000000000000000,0x0000000000000004\n"
".quad 0x0000000000000000,0x000000030000000b,0x0000000000000000,0x0000000000000000\n"
".quad 0x000000000000033b,0x0000000000000016,0x0000000000000000,0x0000000000000001\n"
".quad 0x0000000000000000,0x0000000200000013,0x0000000000000000,0x0000000000000000\n"
".quad 0x0000000000000351,0x0000000000000120,0x0000000b00000002,0x0000000000000001\n"
".quad 0x0000000000000018,0x0000000100000036,0x0000000000100006,0x0000000000000000\n"
".quad 0x0000000000000471,0x00000000000002f0,0x1400000600000003,0x0000000000000004\n"
".quad 0x0000000000000000,0x0000000100000098,0x0000000000000002,0x0000000000000000\n"
".quad 0x0000000000000761,0x0000000000000040,0x0000000400000000,0x0000000000000004\n"
".quad 0x0000000000000000,0x0000000100000051,0x0000000000000002,0x0000000000000000\n"
".quad 0x00000000000007a1,0x000000000000006c,0x0000000400000000,0x0000000000000001\n"
".quad 0x2b366c72255b202c,0x616d660a3b5d3635,0x203233662e6e722e,0x6625202c31356625\n"
".quad 0x39346625202c3035,0x0a3b38346625202c,0x65726168732e646c,0x6625203233662e64\n"
".quad 0x6c72255b202c3235,0x0a3b5d3036392b37,0x65726168732e646c,0x6625203233662e64\n"
".quad 0x6c72255b202c3335,0x660a3b5d30362b36,0x33662e6e722e616d,0x202c353566252032\n"
".quad 0x6625202c33356625,0x31356625202c3235,0x3220636f6c2e0a3b,0x620a312031323120\n"
".quad 0x20636e79732e7261,0x20636f6c2e0a3b30,0x0a36352031392032,0x203233732e646461\n"
".quad 0x7225202c34377225,0x3b387225202c3437,0x3233732e6464610a,0x25202c3537722520\n"
".quad 0x3b3631202c353772,0x203220636f6c2e0a,0x7465730a31203139,0x3233732e656c2e70\n"
".quad 0x7225202c32702520,0x3b367225202c3537,0x726220327025400a,0x3b325f3042422061\n"
".quad 0x696e752e6172620a,0x0a3b345f30424220,0x0a3a335f3042420a,0x203233662e766f6d\n"
".quad 0x6630202c35356625,0x3030303030303030,0x345f3042420a0a3b,0x3220636f6c2e0a3a\n"
".quad 0x6f6d0a3120343620,0x7225203233752e76,0x61746325202c3237,0x68730a3b792e6469\n"
".quad 0x7225203233622e6c,0x32377225202c3835,0x6f6c2e0a3b34202c,0x3120383620322063\n"
".quad 0x3233752e766f6d0a,0x25202c3935722520,0x610a3b792e646974,0x25203233732e6464\n"
".quad 0x357225202c303672,0x3b38357225202c39,0x203220636f6c2e0a,0x766f6d0a31203336\n"
".quad 0x377225203233752e,0x6961746325202c33,0x6c68730a3b782e64,0x367225203233622e\n"
".quad 0x2c33377225202c32,0x636f6c2e0a3b3420,0x0a31203736203220,0x203233752e766f6d\n"
".quad 0x7425202c33367225,0x6c2e0a3b782e6469,0x373231203220636f,0x732e6464610a3120\n"
".quad 0x2c34367225203233,0x25202c3336722520,0x2e646c0a3b323672,0x33752e6d61726170\n"
".quad 0x202c383672252032,0x7274616d395a5f5b,0x5366506c754d7869,0x61705f69695f535f\n"
".quad 0x0a3b5d345f6d6172,0x31203220636f6c2e,0x64616d0a31203732,0x203233732e6f6c2e\n"
".quad 0x7225202c35367225,0x38367225202c3036,0x0a3b34367225202c,0x6d617261702e646c\n"
".quad 0x6c7225203436752e,0x395a5f5b202c3232,0x754d78697274616d,0x695f535f5366506c\n"
".quad 0x5f6d617261705f69,0x617476630a3b5d30,0x626f6c672e6f742e,0x25203436752e6c61\n"
".quad 0x7225202c39316c72,0x6f6c2e0a3b32326c,0x2037323120322063,0x69772e6c756d0a31\n"
".quad 0x25203233732e6564,0x7225202c30326c72,0x610a3b34202c3536,0x25203436732e6464\n"
".quad 0x7225202c31326c72,0x6c7225202c39316c,0x672e74730a3b3032,0x33662e6c61626f6c\n"
".quad 0x31326c72255b2032,0x3b35356625202c5d,0x203220636f6c2e0a,0x65720a3220383231\n"
".quad 0x000a0a0a7d0a3b74\n"
".text");

extern "C" {

extern const unsigned long long fatbinData[861];
...
# 61 "./tests/MatrixMul/matrixMul_kernel.cu"
