...
template<class T, const int pointsPerThread>
__global__ void Mandelbrot0_sm13(uchar4 *dst, const int imageW, 
	const int imageH, const int crunch, const T xOff, const T yOff, 
	const T xJP, const T yJP, const T scale, const uchar4 colors,
	const int frame, const int animationFrame, const int gridWidth,
   ...
   // loop until all blocks completed
    while(blockIndex < numBlocks) {
        ...
        if ((points > 0) && (iy < imageH)) {
			...
			T xPos[pointsPerThread];
		    int m[pointsPerThread];
		    
		    for(int i = 0; i < points; ++i) {
		    	xPos[i] = (T)(ix + i) * scale + xOff;	    	
		    }
    		
            // Calculate the Mandelbrot index for the current location
            CalcMandelbrot<T>(m, xPos, yPos, xJP, yJP, crunch, isJ, points);
//            int m = blockIdx.x;         // uncomment to see scheduling order
		    for(int i = 0; i < points; ++i) {
	            m[i] = m[i] > 0 ? crunch - m[i] : 0;
					
		        // Convert the Mandelbrot index into a color
		        uchar4 color;
		        if (m[i]) {
					m[i] += animationFrame;
					color.x = m[i] * colors.x;
					color.y = m[i] * colors.y;
					color.z = m[i] * colors.z;
				} else {
					color.x = 0;
					color.y = 0;
					color.z = 0;
						 ...
						 const int animationFrame, const int mode, const int numSMs, const bool isJ)
{
    ...
    default:
    case 0:
	    Mandelbrot0_sm13<float, 16><<<numWorkerBlocks, threads>>>(dst, imageW, imageH, crunch, (float)xOff, (float)yOff, 
    ...
    case 1:
        ...
        break;
    case 2:
		Mandelbrot0_sm13<double, 16><<<numWorkerBlocks, threads>>>(dst, imageW, imageH, crunch, xOff, yOff, 
