# 1 "/tmp/tmpxft_00003483_00000000-1_bandwidthTest.cudafe1.cpp"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/tmp/tmpxft_00003483_00000000-1_bandwidthTest.cudafe1.cpp"
# 1 "bandwidthTest.cu"
...
extern void runTest(const int, const char **);
extern void testBandwidth(unsigned, unsigned, unsigned, testMode, memcpyKind, printMode, memoryMode, int, int, bool);

extern void testBandwidthQuick(unsigned, memcpyKind, printMode, memoryMode, int, int, bool);
extern void testBandwidthRange(unsigned, unsigned, unsigned, memcpyKind, printMode, memoryMode, int, int, bool);

extern void testBandwidthShmoo(memcpyKind, printMode, memoryMode, int, int, bool);
extern float testDeviceToHostTransfer(unsigned, memoryMode, bool);
extern float testHostToDeviceTransfer(unsigned, memoryMode, bool);
extern float testDeviceToDeviceTransfer(unsigned);
extern void printResultsReadable(unsigned *, float *, unsigned);
extern void printResultsCSV(unsigned *, float *, unsigned);
extern void printHelp();
# 108 "bandwidthTest.cu"
...
{
auto int start = (32 * (1 << 20));
auto int end = (32 * (1 << 20));
auto int startDevice = 0;
auto int endDevice = 0;
auto int increment = (1 << 22);
auto testMode mode = QUICK_MODE;
auto bool htod = false;
auto bool dtoh = false;
auto bool dtod = false;
auto bool wc = false;
auto char *modeStr;
auto char *device = (__null);
auto printMode printmode = USER_READABLE;
auto char *memModeStr = (__null);
auto memoryMode memMode = PAGEABLE;
...
auto unsigned count = ((1) + (end - start) / increment);

auto unsigned *memSizes = ((unsigned *)malloc(count * sizeof(unsigned)));
auto float *bandwidths = ((float *)malloc(count * sizeof(float)));
...
auto unsigned count = (((((((1 + ((20 * (1 << 10)) / (1 << 10))) + (((50 * (1 << 10)) - (20 * (1 << 10))) / (1 << 11))) + (((100 * (1 << 10)) - (50 * (1 << 10))) / (10 * (1 << 10)))) + (((1 << 20) - (100 * (1 << 10))) / (100 * (1 << 10)))) + (((1 << 24) - (1 << 20)) / (1 << 20))) + (((1 << 25) - (1 << 24)) / (1 << 21))) + (((1 << 26) - (1 << 25)) / (1 << 22)));
# 480 "bandwidthTest.cu"
auto unsigned *memSizes = ((unsigned *)malloc(count * sizeof(unsigned)));
auto float *bandwidths = ((float *)malloc(count * sizeof(float)));
...
auto int iteration = 0;
auto unsigned memSize = (0);
while (memSize <= (1 << 26))
{
if (memSize < (20 * (1 << 10)))
{
...
# 557 "bandwidthTest.cu"
float testDeviceToHostTransfer(unsigned memSize, memoryMode memMode, bool wc)
{
auto unsigned timer = (0);
auto float elapsedTimeInMs = (0.0F);
auto float bandwidthInMBs = (0.0F);
auto unsigned char *h_idata = (__null);
auto unsigned char *h_odata = (__null);
auto cudaEvent_t start; auto cudaEvent_t stop;

__cutilCheckError(cutCreateTimer(&timer), "bandwidthTest.cu", 566);
__cudaSafeCall(cudaEventCreate(&start), "bandwidthTest.cu", 567);
__cudaSafeCall(cudaEventCreate(&stop), "bandwidthTest.cu", 568);
...
__cudaSafeCall(cudaHostAlloc((void **)(&h_idata), memSize, wc ? 4 : 0), "bandwidthTest.cu", 575);
__cudaSafeCall(cudaHostAlloc((void **)(&h_odata), memSize, wc ? 4 : 0), "bandwidthTest.cu", 576);
...
auto unsigned char *d_idata;
__cudaSafeCall(cudaMalloc((void **)(&d_idata), memSize), "bandwidthTest.cu", 596);


__cudaSafeCall(cudaMemcpy(d_idata, h_idata, memSize, cudaMemcpyHostToDevice), "bandwidthTest.cu", 600);



__cutilCheckError(cutStartTimer(timer), "bandwidthTest.cu", 603);
__cudaSafeCall(cudaEventRecord(start, 0), "bandwidthTest.cu", 604);
if ((PINNED) == memMode)
{
for (unsigned i = (0); i < (10); i++)
{
__cudaSafeCall(cudaMemcpyAsync(h_odata, d_idata, memSize, cudaMemcpyDeviceToHost, 0), "bandwidthTest.cu", 610);
...
for (unsigned i = (0); i < (10); i++)
{
__cudaSafeCall(cudaMemcpy(h_odata, d_idata, memSize, cudaMemcpyDeviceToHost), "bandwidthTest.cu", 618);

}
}
__cudaSafeCall(cudaEventRecord(stop, 0), "bandwidthTest.cu", 621);


__cudaSafeCall(cudaThreadSynchronize(), "bandwidthTest.cu", 624);

__cutilCheckError(cutStopTimer(timer), "bandwidthTest.cu", 626);
__cudaSafeCall(cudaEventElapsedTime(&elapsedTimeInMs, start, stop), "bandwidthTest.cu", 627);
...
__cudaSafeCall(cudaEventDestroy(stop), "bandwidthTest.cu", 638);
__cudaSafeCall(cudaEventDestroy(start), "bandwidthTest.cu", 639);
__cutilCheckError(cutDeleteTimer(timer), "bandwidthTest.cu", 640);
if ((PINNED) == memMode)
{
__cudaSafeCall(cudaFreeHost(h_idata), "bandwidthTest.cu", 643);
__cudaSafeCall(cudaFreeHost(h_odata), "bandwidthTest.cu", 644);
} else
...
__cudaSafeCall(cudaFree(d_idata), "bandwidthTest.cu", 651);

return bandwidthInMBs;
}
# 660 "bandwidthTest.cu"
...
auto float bandwidthInMBs = (0.0F);
auto cudaEvent_t start; auto cudaEvent_t stop;
__cutilCheckError(cutCreateTimer(&timer), "bandwidthTest.cu", 666);
__cudaSafeCall(cudaEventCreate(&start), "bandwidthTest.cu", 667);
__cudaSafeCall(cudaEventCreate(&stop), "bandwidthTest.cu", 668);
...
__cudaSafeCall(cudaHostAlloc((void **)(&h_odata), memSize, wc ? 4 : 0), "bandwidthTest.cu", 676);
...
auto unsigned char *h_cacheClear1 = ((unsigned char *)malloc(1 << 24));
auto unsigned char *h_cacheClear2 = ((unsigned char *)malloc(1 << 24));
...
auto unsigned char *d_idata;
__cudaSafeCall(cudaMalloc((void **)(&d_idata), memSize), "bandwidthTest.cu", 702);

__cutilCheckError(cutStartTimer(timer), "bandwidthTest.cu", 704);
__cudaSafeCall(cudaEventRecord(start, 0), "bandwidthTest.cu", 705);

if ((PINNED) == memMode)
{
for (unsigned i = (0); i < (10); i++)
{
__cudaSafeCall(cudaMemcpyAsync(d_idata, h_odata, memSize, cudaMemcpyHostToDevice, 0), "bandwidthTest.cu", 712);
...
for (unsigned i = (0); i < (10); i++)
{
__cudaSafeCall(cudaMemcpy(d_idata, h_odata, memSize, cudaMemcpyHostToDevice), "bandwidthTest.cu", 719);

}
}

__cudaSafeCall(cudaEventRecord(stop, 0), "bandwidthTest.cu", 723);
__cudaSafeCall(cudaThreadSynchronize(), "bandwidthTest.cu", 724);

__cutilCheckError(cutStopTimer(timer), "bandwidthTest.cu", 726);
__cudaSafeCall(cudaEventElapsedTime(&elapsedTimeInMs, start, stop), "bandwidthTest.cu", 727);
if (((PINNED) != memMode) || bDontUseGPUTiming)
{
elapsedTimeInMs = cutGetTimerValue(timer);
}
__cutilCheckError(cutResetTimer(timer), "bandwidthTest.cu", 732);
...
__cudaSafeCall(cudaEventDestroy(stop), "bandwidthTest.cu", 739);
__cudaSafeCall(cudaEventDestroy(start), "bandwidthTest.cu", 740);
__cutilCheckError(cutDeleteTimer(timer), "bandwidthTest.cu", 741);
if ((PINNED) == memMode)
{
__cudaSafeCall(cudaFreeHost(h_odata), "bandwidthTest.cu", 744);
} else
...
free(h_cacheClear1);
free(h_cacheClear2);
__cudaSafeCall(cudaFree(d_idata), "bandwidthTest.cu", 752);

return bandwidthInMBs;
}
# 761 "bandwidthTest.cu"
...
__cutilCheckError(cutCreateTimer(&timer), "bandwidthTest.cu", 768);
__cudaSafeCall(cudaEventCreate(&start), "bandwidthTest.cu", 769);
__cudaSafeCall(cudaEventCreate(&stop), "bandwidthTest.cu", 770);


auto unsigned char *h_idata = ((unsigned char *)malloc(memSize));
...
auto unsigned char *d_idata;
__cudaSafeCall(cudaMalloc((void **)(&d_idata), memSize), "bandwidthTest.cu", 782);
auto unsigned char *d_odata;
__cudaSafeCall(cudaMalloc((void **)(&d_odata), memSize), "bandwidthTest.cu", 784);


__cudaSafeCall(cudaMemcpy(d_idata, h_idata, memSize, cudaMemcpyHostToDevice), "bandwidthTest.cu", 788);



__cutilCheckError(cutStartTimer(timer), "bandwidthTest.cu", 791);
__cudaSafeCall(cudaEventRecord(start, 0), "bandwidthTest.cu", 792);
for (unsigned i = (0); i < (10); i++)
{
__cudaSafeCall(cudaMemcpy(d_odata, d_idata, memSize, cudaMemcpyDeviceToDevice), "bandwidthTest.cu", 796);

}
__cudaSafeCall(cudaEventRecord(stop, 0), "bandwidthTest.cu", 798);




__cudaSafeCall(cudaThreadSynchronize(), "bandwidthTest.cu", 803);


__cutilCheckError(cutStopTimer(timer), "bandwidthTest.cu", 806);
__cudaSafeCall(cudaEventElapsedTime(&elapsedTimeInMs, start, stop), "bandwidthTest.cu", 807);
...
__cutilCheckError(cutDeleteTimer(timer), "bandwidthTest.cu", 818);
free(h_idata);
__cudaSafeCall(cudaEventDestroy(stop), "bandwidthTest.cu", 820);
__cudaSafeCall(cudaEventDestroy(start), "bandwidthTest.cu", 821);
__cudaSafeCall(cudaFree(d_idata), "bandwidthTest.cu", 822);
__cudaSafeCall(cudaFree(d_odata), "bandwidthTest.cu", 823);
...
# 1 "/tmp/tmpxft_00003483_00000000-1_bandwidthTest.cudafe1.stub.c" 1

extern "C" {

# 1 "/tmp/tmpxft_00003483_00000000-3_bandwidthTest.fatbin.c" 1
...
# 2 "/tmp/tmpxft_00003483_00000000-3_bandwidthTest.fatbin.c" 2
...
0x6d742f706d742f20ull,0x3030305f74667870ull,0x30305f3338343330ull,0x372d303030303030ull,
0x646977646e61625full,0x632e747365546874ull,0x2f2820692e337070ull,0x494263632f706d74ull,
0x6467795337582e23ull,0x2d2d2d2f2f090a29ull,0x2d2d2d2d2d2d2d2dull,0x2d2d2d2d2d2d2d2dull,
0x2d2d2d2d2d2d2d2dull,0x2d2d2d2d2d2d2d2dull,0x2d2d2d2d2d2d2d2dull,0x2d2d2d2d2d2d2d2dull,
...
0x742f22093209656cull,0x6678706d742f706dull,0x3433303030305f74ull,0x30303030305f3338ull,
0x61625f362d303030ull,0x546874646977646eull,0x616475632e747365ull,0x227570672e326566ull,
...
0x656c69662e090a22ull,0x73752f2209353109ull,0x2f6c61636f6c2f72ull,0x6e69622f61647563ull,
...
0x6563697665642f65ull,0x6f6974636e75665full,0x2e090a22682e736eull,0x09393109656c6966ull,
0x6f6c2f7273752f22ull,0x616475632f6c6163ull,0x2f2e2e2f6e69622full,0x2f6564756c636e69ull,
0x6e6f635f6874616dull,0x682e73746e617473ull,0x656c69662e090a22ull,0x73752f2209303209ull,
0x2f6c61636f6c2f72ull,0x6e69622f61647563ull,0x6c636e692f2e2e2full,0x315f6d732f656475ull,
0x63696d6f74615f31ull,0x6f6974636e75665full,0x2e090a22682e736eull,0x09313209656c6966ull,
0x6f6c2f7273752f22ull,0x616475632f6c6163ull,0x2f2e2e2f6e69622full,0x2f6564756c636e69ull,
...
0x0a22682e33787470ull,0x0000000000000a0aull
};
...
static __cudaFatCudaBinary __fatDeviceText __attribute__ ((section (".nvFatBinSegment")))= {0x1ee55a01,0x00000003,0x8ecc680c,(char*)"d178f259804f17d8",(char*)"bandwidthTest.cu",(char*)" ",__ptxEntries,__cubinEntries,&__debugEntries0,0,0,0,0,0,0x539451c8};
# 5 "/tmp/tmpxft_00003483_00000000-1_bandwidthTest.cudafe1.stub.c" 2
...
# 6 "/tmp/tmpxft_00003483_00000000-1_bandwidthTest.cudafe1.stub.c" 2
static void __sti____cudaRegisterAll_48_tmpxft_00003483_00000000_4_bandwidthTest_cpp1_ii_main(void) __attribute__((__constructor__));
static void __sti____cudaRegisterAll_48_tmpxft_00003483_00000000_4_bandwidthTest_cpp1_ii_main(void){__cudaFatCubinHandle = __cudaRegisterFatBinary((void*)(&__fatDeviceText)); atexit(__cudaUnregisterBinaryUtil);}

}
# 903 "bandwidthTest.cu" 2
