...
/* 
    ...
    Why not use RGBA8 ? CUDA cannot consistently work with this format, at least for now. 
    Older HW is not storing data the same way as newer HW, leading to inverted R and B components
    On older HW. Workarounds to come...
...
void processImage()
{
    ...
    // 2 solutions, here :
    // - use glTexSubImage2D()... but with the risk of loosing perfs in possible hidden conversion
...
{
    if (animate) {
        rotate[0] += 0.2; if( rotate[0] > 360.0 ) rotate[0] -= 360.0f;
        rotate[1] += 0.6; if( rotate[1] > 360.0 ) rotate[1] -= 360.0f;
        rotate[2] += 1.0; if( rotate[2] > 360.0 ) rotate[2] -= 360.0f;    
	}
    glutPostRedisplay();
}
