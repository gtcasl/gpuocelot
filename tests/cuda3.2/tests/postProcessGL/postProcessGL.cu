		   ...
		   int radius, float threshold, float highlight)
{
...
#if 0 
    shrLog("CUDA Array channel descriptor, bits per component:\n");
    shrLog("X %d Y %d Z %d W %d, kind %d\n", 
    desc.x,desc.y,desc.z,desc.w,desc.f);

    shrLog("Possible values for channel format kind: i %d, u%d, f%d:\n",
    ...
    {
        shrLog("\n");
        shrLogEx( LOGBOTH | MASTER, 0, "postprocessGL, Throughput = %.4f MTexels/s, Time = %.5f s, Size = %.0f Texels, NumDevsUsed = %d, Workgroup = %u\n", 
