...
{
    printf("Loading \"%s\" ...\n", name);
    char *name_ = cutFindFilePath(name, exePath);
    if (!name_)
    {
	...
	CUTBoolean result = cutLoadPPM4ub(name_, &data, &w, &h);
...
int main(int argc, char **argv)
{
    ...
    {
        shrQAFinish(argc, const_cast<const char **>(argv), QA_FAILED);
        return -1;
    }
    ...
    shrQAFinish(argc, const_cast<const char **>(argv), status ? QA_PASSED : QA_FAILED);
