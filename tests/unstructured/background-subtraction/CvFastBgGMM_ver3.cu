...
CvFastBgGMM* cvCreateFastBgGMM(CvFastBgGMMParams* pGMMParams, IplImage* initFrame)
{
	...
	h_pGMMRet->nInputImgSize = 4 * initFrame->width *  pGMMParams->nHeight;
...
void cvUpdateFastBgGMM(CvFastBgGMM* pGMM, IplImage* inputImg)
{
	...
	cudaMemcpyAsync(pGMM->d_inputImg, pGMM->h_pinnedIn, pGMM->nInputImgSize, cudaMemcpyHostToDevice, pGMM->copyStream);
	cudaStreamSynchronize(pGMM->copyStream);

	cudaUpdateFastBgGMM<<< (pGMM->nBlocksPerGrid), (pGMM->nThreadsPerBlock), 4 >>>
		( pGMM->d_inputImg, pGMM->d_outputImg );
	CUDAGMM_SAFE_CALL( cudaThreadSynchronize() );

	cudaMemcpyAsync(pGMM->h_pinnedOut, pGMM->d_outputImg, pGMM->nOutputImgSize, cudaMemcpyDeviceToHost, pGMM->copyStream);
	cudaStreamSynchronize(pGMM->copyStream);
